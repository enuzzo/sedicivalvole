...
- 🧱 ATLAS has tested, session-only A3/A3b/A4 foundations: eight monotonic GPS samples, timestamp-based 30/60 FPS interpolation, stale-data freeze, no extrapolation, network-free road-name extraction from rendered tiles, and deterministic English cardinal sectors. The vehicle point, high-rate feed, road badge, compass labels and selected visual treatment are intentionally not connected before the remaining three-direction overlay gate;
