...
- ✅ seven choices in the shared Flux Visual catalogue: six rendered environments—procedural Aperture, byte-identical upstream Interstate 7 Vertigo, the original architectural Meridian environment, lazy-loaded OpenFreeMap ATLAS, source-faithful Rezmason Drivey, and PRTCL—and the separate DISCOVER 07 Passenger Index destination. The initial Instrument Deck lays them out `3–3–1`; the running Visual library fits all seven rows together at `773 × 601` and exposes the same catalogue without pretending that Discover is a renderer;
