...
- 🧱 SOUNDTRACK now has a tested, non-production metadata foundation: a short-lived fail-closed Jamendo catalogue view and immutable previous/current/next rotation with bounded recent memory, fresh-target validation, recovery refill and no automatic fallback; it stores no audio and does not yet fetch, preload or play anything;
