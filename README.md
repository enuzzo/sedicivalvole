...
Local [iPhone motion and phone-to-Tesla feasibility probes](docs/MOTION-FEASIBILITY-2026-09-18.md) now exercise sensor permission/quality and synthetic WebRTC timing. They are excluded from the deployed product; real iPhone/Tesla measurements and a trusted device execution path remain open.
...
> three characters: hybrid Mono and sampled Rosso/Touring; Flux has three adaptive scores, Soundtrack and a Visual
...
> reports every 15 active session minutes, with a persistent OFF switch.
...
**Night Instrument (September 24).** The owner-selected interface direction
turns the chrome into a lit instrument: plates of keys with LED state, an
odometer speed display with a 130 km/h gauge, a sliding Music/Engine switch,
artwork-led footer keys, a gallery Visual library, eight Performance FX pads,
an Engine cluster with shift lights and one full-width TAMARRO, a START that
opens the Signal Gate, and a passenger remote that mirrors the display. Bass
Cut, Mid Focus and High Cut are clean fourth-order filters. See
[Night Instrument](docs/NIGHT-INSTRUMENT-2026-09-24.md).
...
- ✅ music transport glyphs are bundled in the initial application chunk; loading/error states retain their controls. Current, neighboring and displayed track artwork recovers on online/foreground return with bounded retries and republishes ready native artwork. Native Stop safely pauses the selected audio without discarding it.
...
- ✅ DISCOVER is an independent passenger index, not ATLAS chrome: its selected split layout uses a `52 px` heading band with a `24 px` title, `48 px` tools, `64 px` result rows and a `246 px` rail, fills the available fold before an exact inline `+N MORE`, keeps all at-most-15 image-led sources in one scroll, and leaves one complete localized Wikipedia article open at right with readable chapters, images and infoboxes inside a sandbox. It follows the first supported browser language and provides an internal language selector. An empty query keeps session-only `NEARBY / AHEAD / REGION`; free text searches the selected Wikipedia edition globally, preserves Wikipedia relevance order and works without GPS. The destination-only Google Maps QR remains in place with Tesla-app handoff guidance, never setting an origin or starting navigation automatically, and ATLAS now reuses its source data and full-article URL for map POIs;
- ✅ fixed recordings remain at authored `1×` playback and driving never selects or retimes them. Equal-width footer MUTE and FX controls retain direct `LABEL / ON–OFF` hierarchy while obsolete `GLOBAL` and active-count microcopy is removed to protect the compact `72 px` control plane. FX gates only the audible braking UNDERWATER processing while its shared envelope continues to animate the visuals. One shared two-stage perceptual UNDERWATER model serves Soundtrack and NIGHTSHIFT, reaches approximately `1.5 kHz` at the visible engagement threshold and approximately `460 Hz` at full depth, and preserves bounded pressure and output level. PLAY THE ROAD starts enabled and SOUNDTRACK still requires fresh-session opt-in. Eight independent passenger controls—Flanger, Reverb, Underwater, Phaser, Bitcrush, Bass Cut, Mid Focus, and High Cut—live in a footer-launched `2 × 4` FX Deck of performance pads (tap for the hit, drag for depth), and share one limited wet graph across both music sources. Bass Cut, Mid Focus, and High Cut form one clean fourth-order filter family (September 24; formerly Bass Drive and Radio Cut); licences that disallow effects and all ND/unknown records fail closed before playback;
...
Trusted GPS changes in direction now bend Aperture across near, middle and far
depth, curve Meridian's complete corridor, and bank/twist PRTCL's two forms.
...
ATLAS owns map rendering and POI presentation; the separate lazy Stats module reads the App-owned session and existing system telemetry. Scalar totals accumulate before bounded chart reduction. Passenger view changes preserve audio ownership. See [the selected remix and report design](docs/ATLAS-STATS-REPORT-PLAN-2026-09-07.md).
...
- Speed and diagnostic processing never persist coordinates. ATLAS and DISCOVER keep the latest reliable point only in session memory; OpenFreeMap receives only the tile area needed to draw the city, ATLAS POIs and DISCOVER send localized Wikipedia a coarse `0.05°` cell only for empty-query nearby scopes, and its global free-text search sends no position. When GPS height is absent during a running GPS session, the app sends Open-Meteo an approximately `0.01°` rounded cell to retrieve a Copernicus GLO-90 terrain estimate. A bounded session cache and one request per 30 seconds serve Stats independently of the selected visual; hidden/offline sessions pause requests. The service may log query areas under its published privacy policy. Neither the diagnostic report nor local storage contains the position or journey history.
- No third-party analytics are enabled. During this owner-approved development phase, **Dev / AUTO REPORT ON is the default**: coordinate-free technical packets go to the existing project diagnostic mailbox every **15 minutes of active session time**. Intro and Session report expose OFF; it persists across reloads. Standard requires **SEND DIAGNOSTIC**. Stops, absent GPS and offline time count while the app remains active. Hidden/suspended time is excluded, network recovery never sends a backlog burst, and retries are bounded. Existing bounded local collection is retained in both levels.
...
Tap the stationary TAMARRO bar for a varied 3–5 second neutral rev phrase:
...
36 px preset artwork remains readable. The compact sheet stays centered on the
viewport; credits and Support stay at the bottom edges independently. Every visit rolls a new soundtrack genre and visual effect;
...
| ✈️ ADSB.lol contributors | [API documentation](https://www.adsb.lol/docs/open-data/api/) · [Public project](https://github.com/adsblol) | **Air Atlas integration.** Original bounded same-origin aircraft/route adapter, authorized rounded-location forwarding and shared caching. No provider code imported. Data declared ODbL 1.0; no aircraft dataset bundled. |
| 🗺️ MapLibre contributors — MapLibre GL JS | [Project](https://github.com/maplibre/maplibre-gl-js) | **Production dependency, 6.7.0.** Lazy ATLAS, Air Atlas/Fly With and optional report maps, with a bundled module worker. WebGL2 is required; device acceptance remains separate. Its [camera animation example](https://maplibre.org/maplibre-gl-js/docs/examples/animate-map-camera-around-a-point/) informed the follow-camera study. |
...
| 🌍 OpenMapTiles and OpenStreetMap contributors — OpenMapTiles / OpenStreetMap | [Project](https://github.com/openmaptiles/openmaptiles) | **Runtime data lineage.** OpenFreeMap-delivered map schema/data, selectable named OSM POIs and in-map attribution; https://www.openmaptiles.org/ and https://www.openstreetmap.org/copyright. |
| ⛰️ Open-Meteo and the European Union Copernicus programme — Open-Meteo / Copernicus elevation | [Project](https://github.com/open-meteo/open-meteo) | **Runtime elevation data.** A session-only shared cache supplies GLO-90 terrain fallback when GPS height is missing, using approximately 1 km rounded request cells. Stats and explicit Travel Report exports label the map estimate separately from GPS; linked CC BY 4.0 / Copernicus credit is retained. No API implementation is copied. |
| 📚 Wikimedia communities and individual article/media authors — Wikipedia / Wikimedia | [Project](https://www.mediawiki.org/wiki/API:Main_page) | **Runtime content service.** DISCOVER uses localized search, abstracts, PageImages and native articles; ATLAS reuses the source for real-coordinate POIs, photo cards and its in-page reader. No encyclopedia content is bundled. |
...
<!-- NPM-CREDITS:START -->
<details>
<summary>🧩 Every locked npm dependency — 189 entries, including transitive and optional platform packages</summary>
...
| 🧩 @mapbox/unitbezier `1.0.0` | Project contributors (no author field published) | [Repository](https://github.com/mapbox/unitbezier) · [Project page](https://github.com/mapbox/unitbezier) · [Release metadata](https://registry.npmjs.org/%40mapbox%2Funitbezier/1.0.0) | Application dependency tree. unit bezier curve interpolation |
| 🧩 @mapbox/vector-tile `3.0.0` | Project contributors (no author field published) | [Repository](https://github.com/mapbox/vector-tile-js) · [Project page](https://github.com/mapbox/vector-tile-js#readme) · [Release metadata](https://registry.npmjs.org/%40mapbox%2Fvector-tile/3.0.0) | Application dependency tree. Parses vector tiles |
| 🧩 @maplibre/geojson-vt `6.1.1` | Vladimir Agafonkin | [Repository](https://github.com/maplibre/geojson-vt) · [Project page](https://github.com/maplibre/geojson-vt#readme) · [Release metadata](https://registry.npmjs.org/%40maplibre%2Fgeojson-vt/6.1.1) | Application dependency tree. Slice GeoJSON data into vector tiles efficiently |
| 🧩 @maplibre/maplibre-gl-style-spec `26.4.2` | MapLibre | [Repository](https://github.com/maplibre/maplibre-style-spec) · [Project page](https://maplibre.org/maplibre-style-spec/) · [Release metadata](https://registry.npmjs.org/%40maplibre%2Fmaplibre-gl-style-spec/26.4.2) | Application dependency tree. a specification for maplibre styles |
| 🧩 @maplibre/mlt `1.2.1` | Project contributors (no author field published) | [Repository](https://github.com/maplibre/maplibre-tile-spec) · [Project page](https://github.com/maplibre/maplibre-tile-spec/#readme) · [Release metadata](https://registry.npmjs.org/%40maplibre%2Fmlt/1.2.1) | Application dependency tree. This package contains a JavaScript decoder for the experimental MapLibre Tile (MLT) vector tile format. |
...
| 🧩 baseline-browser-mapping `2.11.22` | Project contributors (no author field published) | [Repository](https://github.com/web-platform-dx/baseline-browser-mapping) · [Project page](https://github.com/web-platform-dx/baseline-browser-mapping#readme) · [Release metadata](https://registry.npmjs.org/baseline-browser-mapping/2.11.22) | Build / development. A library for obtaining browser versions with their maximum supported Baseline feature set and Widely Available status. |
| 🧩 browserslist `4.28.9` | Andrey Sitnik | [Repository](https://github.com/browserslist/browserslist) · [Project page](https://github.com/browserslist/browserslist#readme) · [Release metadata](https://registry.npmjs.org/browserslist/4.28.9) | Build / development. Share target browsers between different front-end tools, like Autoprefixer, Stylelint and babel-env-preset |
...
| 🧩 caniuse-lite `1.0.30001810` | Ben Briggs | [Repository](https://github.com/browserslist/caniuse-lite) · [Project page](https://github.com/browserslist/caniuse-lite#readme) · [Release metadata](https://registry.npmjs.org/caniuse-lite/1.0.30001810) | Build / development. A smaller version of caniuse-db, with only the essentials! |
...
| 🧩 electron-to-chromium `1.5.427` | Kilian Valkhof | [Repository](https://github.com/Kilian/electron-to-chromium) · [Project page](https://github.com/Kilian/electron-to-chromium#readme) · [Release metadata](https://registry.npmjs.org/electron-to-chromium/1.5.427) | Build / development. Provides a list of electron-to-chromium version mappings |
...
| 🧩 maplibre-gl `6.7.0` | Project contributors (no author field published) | [Repository](https://github.com/maplibre/maplibre-gl-js) · [Project page](https://maplibre.org/) · [Release metadata](https://registry.npmjs.org/maplibre-gl/6.7.0) | Application dependency tree. BSD licensed community fork of mapbox-gl, a WebGL interactive maps library |
...
| 🧩 node-releases `2.0.55` | Sergey Rubanov | [Repository](https://github.com/chicoxyzzy/node-releases) · [Project page](https://github.com/chicoxyzzy/node-releases#readme) · [Release metadata](https://registry.npmjs.org/node-releases/2.0.55) | Build / development. Node.js releases data |
...
| 🧩 update-browserslist-db `1.3.3` | Andrey Sitnik | [Repository](https://github.com/browserslist/update-db) · [Project page](https://github.com/browserslist/update-db#readme) · [Release metadata](https://registry.npmjs.org/update-browserslist-db/1.3.3) | Build / development. CLI tool to update caniuse-lite to refresh target browsers from Browserslist config |
