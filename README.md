...
- ✅ the running Music drawer is now a complete no-scroll `773 × 601` driving surface: the whole Pace/Genre chip is a one-tap target, fifteen Jamendo genres occupy deliberate `8 + 7` rows, six tracks occupy two rows, and the compact player and credits remain visible together. Play the Road shows only its three playable scores with concise descriptions and individual covers; all 29 Illobo recordings have title-specific artwork from one coherent dark Swiss-modernist family;
...
the fifteen genres occupy two `8 + 7` rows at the Tesla viewport, while six
track rows, the player, and complete credit remain visible without scrolling.
Play the Road lists only JUNCTION, FRACTURE, and NIGHTSHIFT, each with its own
cover and listener-facing description. Its deterministic fresh mix
