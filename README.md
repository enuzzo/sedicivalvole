...
- ✅ DISCOVER is an independent passenger index, not ATLAS chrome: its selected split layout uses a measured `272 px` rail, fills the available fold before an exact inline `+N MORE`, keeps all at-most-15 image-led sources in one scroll, and leaves one complete localized Wikipedia article open at right with readable chapters, images and infoboxes inside a scriptless sandbox. It follows the first supported browser language, provides an internal language selector and search, estimates local distance/time, and hands the selected destination only to Google Maps; there is no ATLAS action. Location remains session-only and only DISCOVER's coarse `0.05°` cell reaches localized Wikipedia;
