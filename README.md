...
- 🧱 its detached discovery model keeps only official Jamendo pace/genre metadata, supports exact five-band AUTO or explicit MANUAL next-track filtering with hysteresis, treats missing GPS as unknown rather than zero, and never replaces the current recording when driving pace changes;
