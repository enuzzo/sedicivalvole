...
- ✅ the exact MIT `@shadergradient/react@2.4.20` stack now powers three owner-selected public visuals—Japanese Mist, Acid Orchard, and Chromatic Silk—through one separate lazy product chunk. The standalone `/shadergradient-lab.html` and authenticated `/lab` workbenches remain available with the same three registered studies, all ten official presets, all geometries and registered shader families, and the complete useful public control surface;
- ✅ Modular Aperture selected after exactly three revised Product Design directions;
- ✅ ten choices in the shared Flux Visual catalogue: nine rendered environments—procedural Aperture, byte-identical upstream Interstate 7 Vertigo, the original architectural Meridian environment, lazy-loaded OpenFreeMap ATLAS, source-faithful Rezmason Drivey, PRTCL, and the three lazy ShaderGradient visuals `JAPANESE MIST 08`, `ACID ORCHARD 09`, and `CHROMATIC SILK 10`—plus the separate DISCOVER 07 Passenger Index destination. The initial Instrument Deck and running Visual library fit the complete catalogue at `773 × 601` without pretending that Discover is a renderer;
...
- ✅ the running shell keeps previous/play-pause/next directly reachable above the footer, registers the supported Media Session actions for vehicle/browser mini-players, and shows a bounded glance-readable title/album/artist notice for each committed track identity. Shared drawers accept cancelable down/right swipe dismissal without weakening CLOSE, Escape, scroll or focus behavior;
- ✅ DISCOVER is an independent passenger index, not ATLAS chrome: its selected split layout uses a measured `272 px` rail, fills the available fold before an exact inline `+N MORE`, keeps all at-most-15 image-led sources in one scroll, and leaves one complete localized Wikipedia article open at right with readable chapters, images and infoboxes inside a scriptless sandbox. It follows the first supported browser language, provides an internal language selector and search, estimates local distance/time, and keeps navigation in place behind a destination-only Google Maps QR plus Tesla-app handoff guidance; there is no ATLAS action. Location remains session-only and only DISCOVER's coarse `0.05°` cell reaches localized Wikipedia;
...
promoted the three exact ShaderGradient LAB starting points as autonomous public
visuals under the retained MIT notice: `JAPANESE MIST 08`, `ACID ORCHARD 09`,
and `CHROMATIC SILK 10`. The upstream package is unmodified and isolated in a
lazy product chunk; project code owns the response mapping and Canvas2D fallback.
A real Tesla session at the verified split viewport will compare the nine rendered visuals across
