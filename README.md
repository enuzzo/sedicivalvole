...
- 🧱 SOUNDTRACK now has tested, non-production metadata, media-deck and transition foundations: a short-lived fail-closed Jamendo catalogue view, immutable previous/current/next rotation, a detached three-element direct-source browser controller, and a revision-guarded 450 ms equal-power skip model that remains continuous through rapid retargeting; it stores no audio and remains disconnected from the launcher/player and production audio bus;
