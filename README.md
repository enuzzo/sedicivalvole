...
- ✅ ATLAS has tested, session-only A3/A3b/A4 navigation: a high-cadence ref feed keeps eight monotonic GPS samples even when numeric speed is unavailable, timestamp-based interpolation is identical at 30/60 FPS with stale-data freeze and no extrapolation, and the interpolated feed drives the visible point. The owner-selected Navigator Plaque reads the road name only from already rendered OpenMapTiles data and combines it with an English cardinal, exact degrees, and a filled direction arrow whose continuous rotation avoids a long spin across north. The owner-selected Live Navigator passenger panel keeps the map dominant while separating browser GPS altitude from Open-Meteo/Copernicus GLO-90 ground elevation, pairing bounded distance/time with `15 MIN`, `1 H`, and bounded all-session speed/ground histories. Speed statistics expose average, moving average, and maximum; terrain state is explicit as live, stale, loading, or unavailable. Compact locality, road, Wikipedia context, two nearby choices and an article QR remain visible without scroll. The `42 × 116 px` midpoint `HIDE INFO / SHOW INFO` handle gives the complete field back to the map. No reverse-geocoding request is added; target-Tesla legibility and motion acceptance remain open;
...
- ✅ DISCOVER is an independent passenger index, not ATLAS chrome: its selected split layout uses a measured `272 px` rail, fills the available fold before an exact inline `+N MORE`, keeps all at-most-15 image-led sources in one scroll, and leaves one complete localized Wikipedia article open at right with readable chapters, images and infoboxes inside a scriptless sandbox. It follows the first supported browser language, provides an internal language selector and search, estimates local distance/time, and hands the selected destination only to Google Maps; there is no ATLAS action. Location remains session-only and only the same coarse `0.05°` cell already used by ATLAS reaches localized Wikipedia;
...
- Speed and diagnostic processing never persist coordinates. ATLAS and DISCOVER keep the latest reliable point only in session memory; OpenFreeMap receives only the tile area needed to draw the city, localized Wikipedia receives a coarse `0.05°` nearby-search cell, and Open-Meteo receives an approximately `0.001°` rounded cell only to return 90 m-resolution Copernicus ground elevation while ATLAS is selected. Neither the diagnostic report nor local storage contains the position or journey history.
