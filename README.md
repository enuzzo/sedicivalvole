...
> passenger destination in one Visual catalogue and is under vehicle validation**. PRTCL 06 and the revised OPEN
...
- ✅ bounded network telemetry supplies the raw REPORT with a deterministic notice state that separates browser connectivity estimates from observed app transfers, failures and recoveries. A compact navbar `NET` cell now exposes only that browser estimate—never invented cellular strength or a continuous ping score—and START remains available with explicit constrained/offline or pending-music copy;
- ✅ SOUNDTRACK now discovers eligible fixed recordings through a short-lived, server-side Jamendo catalogue relay, prepares previous/current/next as three transient browser media elements, prefetches their adjacent covers, and streams the selected track through a same-origin no-store audio relay into the production Web Audio graph. Soundtrack and the adaptive macro detector share one exact `AudioContext` in either launch order, so vehicle UNDERWATER cannot depend on a second suspended context. Every transport start has a ten-second wall-clock deadline with atomic rollback, exact-record disposal and retry instead of an indefinitely pending browser `play()` promise. Remote preparation no longer blocks visual launch: a track that becomes prepared after START resumes automatically. The App and owner LAB expose real artist, title, artwork, licence, Jamendo credit, and direct source links without persisting audio or exposing the credential;
- ✅ the running Music drawer is now a complete no-scroll `773 × 601` driving surface: two persistent horizontal top selectors switch Play the Road/Soundtrack immediately; Jamendo keeps only the larger vertically separated Pace/Genre controls, right-aligned fresh-mix notice, six readable tracks and compact player/credit composition. The owner-selected Play the Road composition follows Generated image 35. All 29 Illobo recordings have title-specific artwork from one coherent dark Swiss-modernist family; the browser receives compact matched WebP derivatives while the 512 px PNG masters remain local for the artist;
- ✅ the running shell keeps previous/play-pause/next directly reachable above the footer, registers the supported Media Session actions for vehicle/browser mini-players, and shows a bounded glance-readable title/album/artist notice for each committed track identity. In Soundtrack the black readout is a `116 px` speed-only module; FLUX, BPM and energy are absent, while the on-screen transport retracts with the footer instead of remaining over the visual. The dark Music drawer presents its retained black Tabler Now Playing source icon in white and keeps `NOW PLAYING` on one line; future LIGHT mode may reuse the unchanged black source. Shared drawers accept cancelable down/right swipe dismissal without weakening CLOSE, Escape, scroll or focus behavior;
- ✅ DISCOVER is an independent passenger index, not ATLAS chrome: its selected split layout uses a measured `38 px` heading, `246 px` rail and `16 px` result-number track, fills the available fold before an exact inline `+N MORE`, keeps all at-most-15 image-led sources in one scroll, and leaves one complete localized Wikipedia article open at right with readable chapters, images and infoboxes inside a sandbox. It follows the first supported browser language, provides an internal language selector and search, estimates local distance/time, and keeps the browser in place behind a Google Maps place-search QR plus Tesla-app handoff guidance; the QR does not set an origin or auto-start navigation, and there is no ATLAS action. Location remains session-only and only DISCOVER's coarse `0.05°` cell reaches localized Wikipedia;
- ✅ fixed recordings remain at authored `1×` playback and driving never selects or retimes them. Equal-width footer MUTE and FX controls share one `LABEL / ON–OFF / GLOBAL` hierarchy; FX gates only the audible OPEN/UNDERWATER/BLOOM processing while road macros continue to animate the visuals. One shared two-stage perceptual UNDERWATER model serves Soundtrack and NIGHTSHIFT, reaches approximately `1.5 kHz` at the visible engagement threshold and approximately `460 Hz` at full depth, and preserves bounded pressure and output level. PLAY THE ROAD starts enabled and SOUNDTRACK still requires fresh-session opt-in. Eight independent passenger controls—Flanger, Reverb, Underwater, Phaser, Bitcrush, Bass Drive, Radio Cut, and High Cut—live in a compact footer-launched `2 × 4` FX Deck, use deliberately strong tap states plus individual depth controls, and share one limited wet graph across both music sources. Bass Drive, Radio Cut, and High Cut form one visually marked tone/filter family; licences that disallow effects and all ND/unknown records fail closed before playback;
...
- Speed and diagnostic processing never persist coordinates. ATLAS and DISCOVER keep the latest reliable point only in session memory; OpenFreeMap receives only the tile area needed to draw the city, DISCOVER sends localized Wikipedia a coarse `0.05°` nearby-search cell, and ATLAS sends Open-Meteo an approximately `0.001°` rounded cell only to return 90 m-resolution Copernicus ground elevation while selected. Neither the diagnostic report nor local storage contains the position or journey history.
