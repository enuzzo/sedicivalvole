...
- 🧱 SOUNDTRACK now has tested, non-production metadata and media-deck foundations: a short-lived fail-closed Jamendo catalogue view, immutable previous/current/next rotation, and a detached three-element browser controller that uses direct stream URLs, native `preload=auto`, explicit play, stale-promise rejection and destructive source release; it stores no audio and remains disconnected from the launcher/player;
