...
- Keep the `sedicivalvole` wordmark large and integrated into the launch surface, show the injected build identity discreetly, and center the Netmilk Studio / Illobo credit below the CTA. Link Illobo to `github.com/illobo` and expose the public `github.com/enuzzo/sedicivalvole` source directly beneath the credit.
