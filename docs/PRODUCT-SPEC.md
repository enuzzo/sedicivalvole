...
| SOUNDTRACK fixed recordings | model foundation | fail-closed source admission, short-lived session metadata, previous/current/next rotation, and a detached direct-source three-element browser-media controller; proxy, persistent metadata decision, App/audio-bus connection, 450 ms skip crossfade, credit UI and production playback remain pending |
