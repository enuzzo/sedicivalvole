...
| SOUNDTRACK fixed recordings | model foundation | fail-closed source admission, short-lived session metadata, previous/current/next rotation, a detached direct-source three-element browser-media controller, and the revision-guarded 450 ms equal-power skip model; proxy, persistent metadata decision, App/audio-bus/MediaElementSource routing, queue commit, credit UI and production playback remain pending |
