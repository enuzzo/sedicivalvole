...
`VERTIGO 02` is the approved second visual environment and must render the original Interstate 7 scene, not an independent visual translation. The vendored files from upstream commit `e58d58520bc0dfde21f9e14e6a1b8c7f0a2a2a9e` remain byte-identical and retain the complete road, repeated side light sticks, opposing car-light trails, bloom, camera, deep distortion, fog, colors, geometry, and bundled Three.js/post-processing pipeline. The sedicivalvole integration hides only the upstream editorial page shell so the original canvas fills the Tesla viewport. An external bridge maps `0–130 km/h` to the original `speedUpTarget` and FOV controls: zero speed compensates the base clock to a stop, while the road ceiling reaches the original non-boosted `1×` time rate and `150°` FOV. Body-color themes do not recolor Vertigo because that would modify the requested original rendering.
