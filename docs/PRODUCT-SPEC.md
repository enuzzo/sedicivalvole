...
- **Flux**: the current adaptive music system. Speed, acceleration, deceleration, and discrete motion events shape tempo, arrangement, timbre, and space. Its visual family uses animated gradients, chromatic fields, depth/tunnel motion, and optionally abstract procedural road-like geometry in WebGL.
...
Exactly three revised Product Design directions were shown after the luminous-axis rejection. The user selected **Modular Aperture**: an ordered rectangular field with a low black control plane, flat monochrome geometry, and a restrained palette accent. The interface is Braun-influenced, Swiss, minimal, and slightly brutalist, with rectilinear actions softened by the shared `6 px` radius, a locally hosted Space Grotesk variable type system, no circular buttons, and no glassmorphism. The later owner-selected **Road Sheet** direction defines the shared LIGHT and DARK interface appearance. Its current Tesla-calibrated neutrals use `#f4f4f4 / #e6e6e6 / #171a20` in LIGHT and `#0f0f0f / #1f1f1f / #f4f4f4` in DARK, with signal red `#e82127`; these are product tokens calibrated against the supplied references and community Tesla UI, not a claim of an official in-vehicle specification. The owner-selected **Tesla Compact** refinement defines the operational hierarchy across public, diagnostic, owner-LAB, and ShaderGradient surfaces: `13 px` metadata, `14 px` labels, `15 px` body/action copy, `17 px` active names, `22 px` titles, and `32 px` primary values; `48 px` actions and `56 px` primary actions remain independent touch geometry. Both appearances preserve the same layout, grouping, spacing, typography, and interaction hierarchy. Space Grotesk uses `400` for reading text, `500` for primary values, `600` for controls and `700` for operational labels. Orbitron is the sole textual exception: exact `sedicivalvole` project wordmarks use weight `750` and restrained `-0.02em` tracking in the Signal Gate, Instrument Deck and owner LAB. The running top bar always starts with the compact 16 Road mark and never adds a textual wordmark. `PLAY THE ROAD` remains Space Grotesk `600` with zero added tracking. The active visual palette persists locally across reloads and later visits; product-wide `LIGHT`/`DARK`/`AUTO` appearance is a separate preference with an explicit reset path. AUTO chooses between the shared LIGHT and DARK grammars without recolouring or replacing the active visual palette.

The separate X10 `LIGHT`/`DARK`/`AUTO` preference, explicit reset model,
product-wide semantic tokens, and visible top-rail control are implemented.
LIGHT is the fresh-install default. Manual choices persist independently from
the visual palette. AUTO prioritizes a browser scheme only when its change
signal can actually be observed, then derives day/twilight/night from an
already-consented ephemeral position and UTC time, and otherwise uses a safe
fallback. It makes no network request, stores no coordinate or solar result,
and never initiates a GPS permission prompt. Appearance never recolours or
replaces the active visual palette.

Framed controls, cards, panels, and standalone buttons use one restrained `6 px` corner radius rather than completely sharp corners. This is a system token, not a per-component styling choice. The speed readout occupies an exact header-grid cell with the same divider stroke while the chrome is awake; when the header retires, the readout remains fixed in place and gains only the exposed lower corner treatment so it reads as a detached module rather than an unrelated overlay. At `773 × 601`, the running top bar is exactly `64 px` tall and uses the approved **Tesla Balanced Rail**: 16 Road mark, mode, speed, network, appearance, GPS, Discover, and Report. Speed alone owns a two-line numeric hierarchy; every other actionable cell is a centered icon-only or icon-plus-name peer on a shared baseline. The compact product mark and speed remain visible while the secondary rail rests. Network shows only a three-state colour ring plus genuine loading motion; its disclosure carries current app-only transfer values and a bounded 15-minute quality graph. The appearance trigger selects a contrast-safe sun, moon, or sun-moon icon for LIGHT, DARK, or AUTO. Closing a pinned drawer transfers focus back to the experience so the idle retraction timer cannot remain latched to its former trigger.
...
eight-sector Direction History rose, compact Open-Meteo/Copernicus GLO-90
Elevation, and a low Moving/Stopped strip. Each direction sector covers `45°`;
up to five discrete outward tiles encode relative moving-time share, while a
separate needle shows the latest bearing. Samples below `2 km/h` cannot dominate
the rose, and weighted session rollups retain exact sector counts. Larger direct
labels, time/value axes and explicit empty states make each signal readable
without changing its truthful data boundary. One tap
...
`secondary` colours simultaneously. Its sole direct control cycles Normal and palette-coloured Wire rendering
without a dropdown. The bridge fixes the familiar forward camera; old rear/aerial
preferences migrate to it without resetting render mode. Palette selection remains
shared with the rest of the product.
...
light-tinted derivative, admit bounded Play the Road audio-level response while keeping
...
- controlled non-linear, domain-specific road-response curves;
...
- Continuous parameters include speed, private arrangement drive, filters, dynamics, and effect sends.
...
- A continuous restrained motion wave remains underneath the mix; its frequency and gain rise smoothly with acceleration.
...
- The environment system must make variety structural rather than decorative: changing environment may change visual mechanics and musical genre, while every environment consumes the same normalized speed, acceleration, deceleration, Brake, safety, and diagnostic contracts and derives its own bounded response.
- The user-adjustable response threshold is retired. Renderer-specific pressure and score arrangement drive saturate against a fixed `130 km/h` legal-road ceiling; Aperture must reveal unmistakable tunnel depth by approximately `40 km/h` and continue deforming progressively into its full Plaid field. These private response scalars must not appear in the driving UI or diagnostic payload. Low-level `Atmos`, `Harmonics`, and `Pulse` controls remain excluded from the final driving interface.
