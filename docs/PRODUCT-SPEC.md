...
- The two Soundtrack choices have strict source ownership. Jamendo Library alone uses transient API/relay records and exposes pace, genre, cover and exact-track discovery. Illobo Featured alone uses the 29 owner-authorized hosted Illobo web masters; every press chooses a random non-current Illobo track and starts that complete recording at `0:00`, without dropping any of the 29 identities or seeking to a random point. Source kind and selection identifier are part of catalogue-cache identity, so neither choice can reuse the other's records. The browser keeps no offline copy.
