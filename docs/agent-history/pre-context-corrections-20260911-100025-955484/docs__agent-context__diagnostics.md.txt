# Diagnostics, GPS and report privacy
...
Fresh preferences use Dev / AUTO ON; preserve explicit saved OFF/Standard. Show OFF in Intro and Session report. One in-flight send, bounded retries, matching server validation and disclosure, and no synthetic QA packets to the real mailbox. Automatic sending to the existing destination is explicitly authorized. It supersedes manual-only, future-only, ten-minute and driving-only rules, including those in old trace/ledger prose.

Keep technical packets coordinate-free, including height/position exclusions. The debug interaction ledger must omit pointer coordinates, typed searches, GPS coordinates and media URLs. Keep exact sequence, activation source, safe identity, before/after transport state, latency, lifecycle, native invocation and bounded failure detail. Keep the flight recorder bounded in session memory with full-session aggregates when samples rotate.
...
GPS and demo share normalized speed; never synthesize GPS motion. Coordinate-free restrictions apply to technical diagnostics, not authorized map positioning or the explicit precise-route report export option.
...
Travel Report is selected: compact cover, journey summary, graphs and technical appendix. Precise route inclusion is an explicit export option. Preserve immutable preview/download identity, chosen-recipient verification and optional user-requested delivery; remembered recipients clear with Reset Saved State. Do not infer authorization to send other messages.

Consult [diagnostics](../DIAGNOSTICS.md) only for the relevant schema/transport, and [altitude fallback](../ALTITUDE-FALLBACK-2026-09-07.md) for height work.
