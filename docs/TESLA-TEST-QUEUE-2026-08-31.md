...
| `R4-04` | FAIL 2026-08-31 · FIX LIVE `20260831-2005` · RETEST | Toggle OPEN, BLOOM, and UNDERWATER across FRACTURE, JUNCTION, NIGHTSHIFT, Illobo, and Jamendo, then disable global FX. | Each effect is audible but controlled; UNDERWATER is unmistakably dark from its visible engagement while retaining usable level; disabling FX restores dry output in every source. | `8c53e8d` corrects the zeroed Soundtrack macro boundary and gives Soundtrack/NIGHTSHIFT the shared two-stage perceptual brake. Offline real Illobo/Jamendo threshold measurements lose `20.8/18.8 dB` above `4 kHz`; cabin proof remains mandatory. |
...
| `R5-02` | FAIL 2026-08-31 · FIX LIVE `20260831-2034` · RETEST | In MERIDIAN accelerate progressively from rest through urban, medium, motorway and `130 km/h`, then brake firmly and release; briefly confirm VERTIGO 02 at low and medium speed afterward. | MERIDIAN widens perspective perceptibly throughout the complete `0–130` progression rather than only near one threshold; braking and UNDERWATER engagement remain continuous with no stepping, camera snap or field jump. Both visuals remain fluid, readable, correctly themed, and free of degraded audio. | `6a90621` expands base FOV `50° → 116°`, guarantees at least `10°` per `26 km/h` band, and shares frame-rate-independent `0.30 s` acceleration / `0.62 s` braking plus `0.32 s` visual-FX easing across WebGL2 and Canvas2D. |
