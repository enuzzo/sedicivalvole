...
| `R10-00B` | NOT RUN | During any running visual, open the footer VISUAL library, scroll to DISCOVER 07, and press its OPEN row. Close Discover and reopen the Visual library. | Discover opens immediately; closing returns to the same rendered visual, which remains ACTIVE in the library. No false Discover renderer is saved, no blank field appears, and focus/chrome recovery remains normal. | Running Visual destination path and renderer-preservation regression in `e268169`. |
...
| `R10-02` | NOT RUN | Compare NEARBY, AHEAD and REGION; then use the search field. If the index shows a bottom `+N MORE`, expand and collapse it. | Each filter produces a coherent order; search narrows the existing sources without a network race. Five results remain visible initially, the counter states the exact hidden count, and expansion exposes no more than 15 sources without breaking the fixed reader. | Bounded Wikipedia continuation, bearing sort, search and five-plus-counter disclosure in `5c498ac`. |
| `R10-03` | NOT RUN | Select at least three places, compare their distance/time with the route context, then press the Google Maps action for one. | The reader changes with the selected row; distance and drive time are plausible estimates and clearly subordinate. Google Maps opens driving directions to the selected place without exposing an invented current-location origin. | Distance/ETA model and destination-only Maps URL in `5c498ac`. |
| `R10-04` | NOT RUN | Change the internal language selector to Italian and one other supported language, then select a result in each. | Results and article source move to the selected language edition; the selector remains readable and reversible, and Wikipedia attribution/source links follow the active edition. | Fourteen-language selector and localized Wikipedia endpoint in `5c498ac`. |
| `R10-05` | NOT RUN | Open the selected place in ATLAS, return to DISCOVER, then close the surface and touch nothing for five seconds. | DISCOVER and ATLAS remain distinct and reciprocal; the map keeps its route, point, plaque and gestures. Closing returns focus to the visual and header/footer retract after about 4.2 seconds. | DISCOVER/ATLAS handoff and shared post-action focus recovery in `5c498ac` and `9daf8f6`. |
