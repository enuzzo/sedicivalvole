...
- SOUNDTRACK now uses server-side Jamendo catalogue and exact-ID audio relays,
  ...
  **Soundtrack** switch; Play the Road retains the complete adaptive catalogue,
  ...
  physical-Tesla listening, and target-vehicle effect tuning remain pending.
...
- original `MERIDIAN 03`, rebuilt against the selected reference as a low stable corridor of sparse oblique blades and longitudinal shoulder planes. A frame-rate-independent `0.30 s` acceleration / `0.62 s` braking response prevents GPS and braking-FX steps, while the `50–116°` FOV progression remains visibly active across the complete `0–130 km/h` range. Bounded field motion, depth compression, peripheral stretch and parallax carry speed without conventional buildings, cloud slabs or a particle field;
