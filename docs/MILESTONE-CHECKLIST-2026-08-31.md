...
| Fixed Soundtrack | [x] OFFICE IMPLEMENTED · LIVE `20260901-0012` | The equal **Play the Road** / **Soundtrack** drawer, true 29-track Illobo Featured catalogue, separate Jamendo Library with 15 live-verified genre routes, 30-minute rotation, immediate passenger selection, authored `1×`, three-deck media, and global MUTE/FX are implemented. Checkpoint `856232b` replaces repeated text PLAY actions with full-surface one-tap chips and licensed media icons, arranges genres as `8 + 7`, keeps six tracks plus player/credit visible without scrolling at `773 × 601`, limits Play the Road to its three real scores, and adds 3 score covers plus 29 title-specific Illobo covers. Live QA also exposed and `7085941` fixed a suspended-audio-clock case that could leave NEXT stuck on FADING. | Run `R7-01`–`R7-08`, especially degraded-network `R7-03`, effects `R7-06`, source round-trip `R7-07`, and compact-surface acceptance `R7-08`; also run `R8-01`–`R8-02`. |
...
| 7 | OFFICE + TESLA | [ ] OFFICE + CANONICAL PASS · TESLA RETEST | Finish Soundtrack mechanics | The production deck applies the nominal `450 ms` equal-power model, atomic audio-clock attribution and strict three-element ceiling. `1a47e23` gives Featured its own owner-authorized 29-track catalogue; `236f2c9` guarantees random complete-track starts at `0:00`; `57fed11` makes transport and failure state transactional; `137ddeb` and `0660d71` make source changes immediate and reversible. Owner-selected checkpoint `856232b` adds whole-surface one-tap filters, two-row genre/track compositions, compact player/credit, three real Play the Road cards, and 32 coherent covers. Live QA exposed a frozen-audio-clock FADING edge case; `7085941` now settles the target at the wall-clock deadline. Run `R7-01`–`R7-08`. | Complete `526/526` suite, 147-module App / 71-module LAB / Sites build, exact live `773 × 601` Browser QA, protected 174-file publication, 29/29 Illobo verification, pre/postflight, and canonical byte identity pass for build `20260901-0012`. Both paths measure `601/601 px`; Rock changes the playlist; two Illobo NEXT actions settle without residual FADING; Jamendo returns with all 15 genres and six tracks. `R7-01`–`R7-08` remain cabin gates. |
