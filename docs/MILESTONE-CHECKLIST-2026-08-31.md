...
| 8 | OWNER GATE → OFFICE + TESLA | [ ] OFFICE + CANONICAL PASS; TESLA PENDING | Final Illobo identity and Tesla playback title | Both approved supplied marks replace the provisional cover and crossfade every three seconds. Active fixed playback exposes `16 - Artist - Track title` through the page title and restores the product title on pause. | `05a754b`; build `20260831-1320` at `b59e6ec`; byte-identical assets and canonical HTML/JS/CSS; notice/provenance update; complete `490/490` suite; 146-module App / 70-module LAB / Sites build; exact local/live `773 × 601` timed-cover, title, overflow and console PASS. Run `R8-01`–`R8-02` in the Tesla. |
