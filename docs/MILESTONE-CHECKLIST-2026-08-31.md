...
| Fixed Soundtrack | [x] OFFICE IMPLEMENTED · BASE LIVE `20260901-0012` · NAVIGATOR PUBLICATION PENDING | The equal **Play the Road** / **Soundtrack** drawer, true 29-track Illobo Featured catalogue, separate Jamendo Library with 15 live-verified genre routes, 30-minute rotation, immediate passenger selection, authored `1×`, three-deck media, and global MUTE/FX are implemented. Checkpoint `5c498ac` adds the approved Navigator Rail, a vertical Pace rail with readable `5 × 3` genres, correct Lobo artist authorship, playback activity and bounded QR credit on top of the one-tap/covers checkpoint `856232b`. Live QA also exposed and `7085941` fixed a suspended-audio-clock case that could leave NEXT stuck on FADING. | Publish `5c498ac`, then run `R7-01`–`R7-09`, especially degraded-network `R7-03`, effects `R7-06`, source round-trip `R7-07`, compact-surface acceptance `R7-08`, and Navigator/player acceptance `R7-09`; also run `R8-01`–`R8-02`. |
...
| 7 | OFFICE + TESLA | [ ] OFFICE + CANONICAL PASS · TESLA RETEST | Finish Soundtrack mechanics | The production deck applies the nominal `450 ms` equal-power model, atomic audio-clock attribution and strict three-element ceiling. `1a47e23` gives Featured its own owner-authorized 29-track catalogue; `236f2c9` guarantees random complete-track starts at `0:00`; `57fed11` makes transport and failure state transactional; `137ddeb` and `0660d71` make source changes immediate and reversible. Owner-selected checkpoint `856232b` adds whole-surface one-tap filters, two-row track composition, compact player/credit, three real Play the Road cards, and 32 coherent covers. Checkpoint `5c498ac` adds the approved Navigator Rail, correct Featured Artist/Lobo authorship, vertical Pace plus readable `5 × 3` genres, active-playback indicator and bounded QR credits. Live QA exposed a frozen-audio-clock FADING edge case; `7085941` settles the target at the wall-clock deadline. Run `R7-01`–`R7-09`. | Existing build `20260901-0012` passes complete transport/source QA. The new `5c498ac` UI passes 42 focused UI/presentation checks, 148-module App / 71-module LAB / Sites build, and exact local `773 × 601` Browser QA with all content visible and no credit overlap. Publish the combined checkpoint, then run `R7-01`–`R7-09` as cabin gates. |
| 8 | OWNER GATE → OFFICE + TESLA | [ ] OFFICE + CANONICAL PASS; TESLA PENDING | Final Illobo identity and Tesla playback title | Both approved supplied marks replace the provisional cover. They dissolve continuously over four seconds in each direction between a white-on-black solid state and the original black-on-graphite outline state, on an unclipped square field with no cover border or radius. Active fixed playback exposes `16 - Artist - Track title` through the page title and restores the product title on pause. | Base `05a754b`; perceptual correction `6218f98`; true Illobo source correction `1a47e23`; provider-label correction `2c0f5f8`; final build `20260901-0012`; complete `526/526` suite and 29/29 live audio checks. Run `R8-01`–`R8-02` in the Tesla. |
| 9 | OWNER GATE → OFFICE → TESLA | [ ] OFFICE + CANONICAL PASS; TESLA PENDING | Finish ATLAS road/cardinal overlay | The owner selected direction 1, Navigator Plaque, and requested a dynamic arrow beside the cardinal state. Checkpoint `79d9c9b` implements one compact surface with a filled continuously rotating arrow, English cardinal, exact degrees and a road name read only from rendered `transportation_name` vector data. It replaces the duplicate MapLibre compass, adds no reverse geocoding, and leaves the accepted route/dot treatment unchanged. | Three-direction selection recorded; Navigator-specific `493/493` checkpoint and current complete `526/526` suite pass with 147-module App / 71-module LAB / Sites build; final build `20260901-0012` retains the byte-identical Navigator implementation; prior exact live `773 × 601` QA proves `SE 135° → S 184°`, road copy, zero overflow and no live warning/error. Run `R9-01`–`R9-02` in the Tesla. |
| 10 | OWNER GATE → OFFICE → TESLA | [ ] OFFICE IMPLEMENTED `5c498ac` · CANONICAL/TESLA PENDING | DISCOVER passenger companion | Direction 1 is selected and implemented as a split Passenger Index: five image-led places plus `+N MORE` at left, one always-open reader at right, automatic browser language, 14-language selector, search, Nearby/Ahead/Region, bounded continuation to 15 sources, approximate distance/time, Google Maps directions and reciprocal ATLAS action. The same `0.05°` session-only privacy boundary remains; no location enters storage or diagnostics. | Selection and joined Product Design QA recorded in `design-qa.md`; 7 Discover tests and 42 combined UI checks pass; exact local `773 × 601` Browser QA proves 15-result pagination, English/Italian switching, search, reader and Maps links. Publish, then run `R10-01`–`R10-05`. |
