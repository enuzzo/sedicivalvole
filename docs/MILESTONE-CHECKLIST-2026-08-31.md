...
| Fixed Soundtrack | [x] OFFICE IMPLEMENTED · CORRECTED LIVE | The equal **Play the Road** / **Soundtrack** drawer, true 29-track Illobo Featured catalogue, separate Jamendo Library, 30-minute rotation, immediate passenger selection, authored `1×`, three-deck media, and global MUTE/FX are live. Checkpoint `1a47e23` corrects the source-boundary defect: Featured now loads only hosted owner-authorized Illobo recordings and cannot reuse Jamendo catalogue state. | Retest `R7-06`–`R7-07`; run the remaining `R7-01`–`R7-05` and `R8-01`–`R8-02` in the Tesla. |
...
| 7 | OFFICE + TESLA | [ ] SOURCE CORRECTED · CANONICAL PASS; TESLA RETEST | Finish Soundtrack mechanics | The production deck applies the nominal `450 ms` equal-power model, atomic audio-clock attribution and strict three-element ceiling. The earlier Featured implementation was incorrectly a second ordering of the Jamendo catalogue; therefore its `23/23` relay audit was Jamendo evidence, not Illobo verification. Checkpoint `1a47e23` gives Featured its own owner-authorized 29-track catalogue, prevents cross-source cache reuse, keeps every explicit random start within those 29 Illobo recordings, and preserves NEXT/PREVIOUS traversal and effects permission. Run `R7-01`–`R7-07`. | Complete `499/499` and 146-module App / 70-module LAB / Sites build PASS. Build `20260831-1744` passed protected FTP preflight, a 140-file / 212,284,605-byte publication, full SHA-256 verification of all **29/29** remote Illobo tracks, and read-only postflight. The live catalogue contains exactly 29 Illobo identities; all **29/29** return `206 audio/mpeg` with matching total byte size. `R7-06` and `R7-07` require cabin retest. |
| 8 | OWNER GATE → OFFICE + TESLA | [ ] OFFICE + CANONICAL PASS; TESLA PENDING | Final Illobo identity and Tesla playback title | Both approved supplied marks replace the provisional cover. They dissolve continuously over four seconds in each direction between a white-on-black solid state and the original black-on-graphite outline state, on an unclipped square field with no cover border or radius. Active fixed playback exposes `16 - Artist - Track title` through the page title and restores the product title on pause. | Base `05a754b`; perceptual correction `6218f98`; true Illobo source correction `1a47e23`; current build `20260831-1744`; complete `499/499` suite and 29/29 live audio checks. Run `R8-01`–`R8-02` in the Tesla. |
