...
| Fixed Soundtrack | [x] OFFICE IMPLEMENTED · LIVE `20260901-1041` | The equal **Play the Road** / **Soundtrack** drawer, true 29-track Illobo Featured catalogue, separate Jamendo Library with 15 live-verified genre routes, 30-minute rotation, immediate passenger selection, authored `1×`, three-deck media, and global MUTE/FX are implemented. Owner checkpoint `8450109` supersedes the temporary Music Navigator Rail with two horizontal top selectors and records the selected **Generated image 35** Play the Road composition: sampled JUNCTION/NIGHTSHIFT side by side and responsive-generative FRACTURE full-width below. Exact canonical `773 × 601` QA proves both panes fit without scrolling. | Run `R7-01`–`R7-09`, especially degraded-network `R7-03`, effects `R7-06`, source round-trip `R7-07`, compact-surface acceptance `R7-08`, and source-selector/player acceptance `R7-09`; also run `R8-01`–`R8-02`. |
...
- [x] connect independent manual flanger, reverb, chorus, and bounded echo;
...
| 7 | OFFICE + TESLA | [ ] OFFICE + CANONICAL PASS · TESLA RETEST | Finish Soundtrack mechanics | The production deck applies the nominal `450 ms` equal-power model, atomic audio-clock attribution and strict three-element ceiling. `1a47e23` gives Featured its own owner-authorized 29-track catalogue; `236f2c9` guarantees random complete-track starts at `0:00`; `57fed11` makes transport and failure state transactional; `137ddeb` and `0660d71` make source changes immediate and reversible. Owner-selected checkpoint `856232b` adds whole-surface one-tap filters, compact player/credit and 32 coherent covers. Checkpoint `5c498ac` adds correct Featured Artist/Lobo authorship, vertical Pace plus readable `5 × 3` genres, active-playback indicator and bounded QR credits. Owner checkpoint `8450109` records Generated image 35, restores the two horizontal source selectors, places sampled JUNCTION/NIGHTSHIFT side by side and responsive-generative FRACTURE full-width. Live QA exposed a frozen-audio-clock FADING edge case; `7085941` settles the target at the wall-clock deadline. Run `R7-01`–`R7-09`. | Build `20260901-1041` passes the complete `533/533` suite, 148-module App / 71-module LAB / Sites build, protected publication, 29/29 Illobo hashes, canonical byte identity, exact live `773 × 601` zero-scroll geometry for both panes and an empty Browser console. `R7-01`–`R7-09` remain cabin gates. |
| 8 | OWNER GATE → OFFICE + TESLA | [ ] OFFICE + CANONICAL PASS; TESLA PENDING | Final Illobo identity and Tesla playback title | Both approved supplied marks replace the provisional cover. They dissolve continuously over four seconds in each direction between a white-on-black solid state and the original black-on-graphite outline state, on an unclipped square field with no cover border or radius. Active fixed playback exposes `16 - Artist - Track title` through the page title and restores the product title on pause. | Base `05a754b`; perceptual correction `6218f98`; true Illobo source correction `1a47e23`; provider-label correction `2c0f5f8`; final build `20260901-0933`; complete `533/533` suite and 29/29 live audio checks. Run `R8-01`–`R8-02` in the Tesla. |
| 9 | OWNER GATE → OFFICE → TESLA | [ ] OFFICE + CANONICAL PASS · TESLA PENDING | Finish ATLAS Navigator and passenger panel | The owner selected Navigator Plaque direction 1, then Live Navigator direction 3. Checkpoints `79d9c9b`, `3ce5c98`, and `7debfe3` provide the dynamic arrow/cardinal/degrees/rendered road plaque plus a map-dominant `272 px` LIVE MOTION / WHERE YOU ARE panel. Browser GPS altitude remains distinct from Open-Meteo/Copernicus GLO-90 ground elevation. One tap cycles `15 MIN → 1 H → SESSION`; speed history reports average, moving average above `2 km/h`, and maximum, while ground history reports its range. One-hour detail and bounded weighted session rollups remain session-only. Terrain state is explicit; the `42 × 116 px` `HIDE INFO / SHOW INFO` handle is never a full-height rail. | Build `20260901-1438` at `7debfe3` passes focused `34/34`, complete `539/539`, 148-module App / 71-module LAB / Sites build, protected publication, 29/29 Illobo hashes, canonical five-asset byte identity and exact live `773 × 601` QA. Real Open-Meteo CORS returned HTTP 200; range cycling, `465/465 px` fit, open/collapsed/reopened geometry, localized content, QR and full-map restoration pass. Run `R9-01`–`R9-05` in the Tesla. |
| 10 | OWNER GATE → OFFICE → TESLA | [ ] OFFICE + CANONICAL PASS · TESLA PENDING | DISCOVER passenger companion | Direction 1 is selected and implemented as a split Passenger Index: five image-led places plus `+N MORE` at left, one always-open reader at right, automatic browser language, 14-language selector, search, Nearby/Ahead/Region, bounded continuation to 15 sources, approximate distance/time, Google Maps directions and reciprocal ATLAS action. Checkpoint `e268169` additionally registers **DISCOVER 07** as the seventh driver-facing Visual destination in both the initial Instrument Deck and running Visual library. The splash grid is `3–3–1`; START opens Discover over Aperture, and closing it returns to a real renderer. Follow-up `a257e0c` fits all seven running Visual rows together at `773 × 601` without drawer scrolling. The same `0.05°` session-only privacy boundary remains; no location enters storage or diagnostics. | Build `20260901-1105` passes the complete `536/536` suite, 148-module App / 71-module LAB / Sites build, protected publication, pre/postflight `remote_writes=NONE`, 29/29 Illobo hashes, canonical HTML/JS/CSS byte identity, both exact live `773 × 601` entry loops, central focus return and an empty Browser log. Run `R10-00A`–`R10-05` in the Tesla. |
