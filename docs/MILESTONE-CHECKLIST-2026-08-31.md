...
| 10 | OWNER GATE → OFFICE → TESLA | [ ] OFFICE + CANONICAL PASS · TESLA PENDING | DISCOVER passenger companion | Direction 1 remains the independent split Passenger Index. Checkpoint `f843ea6` enlarges the complete Discover reading hierarchy, uses a measured `272 px` rail, fills the available fold before an exact inline `+N MORE`, keeps all 15 results in one scroll, loads the complete localized Wikipedia article with chapters/images/infobox inside a scriptless reader, prevents cross-language page-ID collisions, and removes the superseded ATLAS action. Google Maps remains destination-only. Checkpoint `e268169` keeps **DISCOVER 07** in both Visual entry points; the splash grid is `3–3–1`, and closing it returns to the unchanged real renderer. The same `0.05°` session-only privacy boundary remains. | Build `20260901-1524` carries `0d50a88`, passes focused `10/10`, complete `541/541`, 148-module App / 71-module LAB / Sites build, protected publication, pre/postflight `remote_writes=NONE`, 29/29 Illobo hashes, canonical HTML/JS/CSS byte identity and exact live `773 × 601` QA: `272 px` rail, `+10 MORE`, 15-row scroll, complete article and no ATLAS action. Run revised `R10-00A`–`R10-05` in the Tesla. |
