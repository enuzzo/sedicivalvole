...
| 7 | OFFICE + TESLA | [ ] SOURCE/TRANSPORT/PATH ROUND TRIP CORRECTED · UI REFINEMENT PUSHED; TESLA RETEST | Finish Soundtrack mechanics | The production deck applies the nominal `450 ms` equal-power model, atomic audio-clock attribution and strict three-element ceiling. `1a47e23` gives Featured its own owner-authorized 29-track catalogue; `236f2c9` guarantees random complete-track starts at `0:00`; `57fed11` makes transport and failure state transactional; `137ddeb` and `0660d71` make source changes immediate and reversible. Owner-selected checkpoint `856232b` adds whole-surface one-tap filters, two-row genre/track compositions, compact player/credit, three real Play the Road cards, and 32 coherent covers. Run `R7-01`–`R7-08`. | Complete `525/525` suite, 147-module App / 71-module LAB / Sites build and exact local `773 × 601` Browser QA pass. The drawer measures `601/601 px` with zero horizontal or vertical overflow; Rock selection replaces the playlist and synchronizes selected/current/transport state. Canonical publication remains the immediate office gate; `R7-01`–`R7-08` remain cabin gates. |
