...
| 9 | OWNER GATE → OFFICE → TESLA | [ ] REDESIGN GATE + TESLA PENDING | Recast ATLAS as Drive Lab | The published checkpoints `79d9c9b`, `3ce5c98`, and `7debfe3` remain the factual baseline: dynamic heading/road plaque, separate GPS altitude and Open-Meteo/Copernicus GLO-90 terrain elevation, `15 MIN / 1 H / SESSION` histories, speed statistics and a bounded collapse control. The owner has now separated ATLAS from DISCOVER and wants a data-first Drive Lab: journey telemetry and useful/playful graphs take priority; no DISCOVER link or duplicated place-card system. Three replacement directions have been presented. The next handle is an icon-only rectangular tab, slightly larger than its icon. | Build `20260901-1438` at `7debfe3` passes focused `34/34`, complete `539/539`, canonical publication and exact live `773 × 601` QA. Wait for owner choice `1 / 2 / 3` before replacing the published panel; then rerun office/canonical QA and `R9-01`–`R9-05` in the Tesla. |
| 10 | OWNER GATE → OFFICE → TESLA | [ ] LOCAL PASS · CANONICAL + TESLA PENDING | DISCOVER passenger companion | Direction 1 remains the independent split Passenger Index. Checkpoint `f843ea6` enlarges the complete Discover reading hierarchy, uses a measured `272 px` rail, fills the available fold before an exact inline `+N MORE`, keeps all 15 results in one scroll, loads the complete localized Wikipedia article with chapters/images/infobox inside a scriptless reader, prevents cross-language page-ID collisions, and removes the superseded ATLAS action. Google Maps remains destination-only. Checkpoint `e268169` keeps **DISCOVER 07** in both Visual entry points; the splash grid is `3–3–1`, and closing it returns to the unchanged real renderer. The same `0.05°` session-only privacy boundary remains. | Local `773 × 601` Browser QA proves `+10 MORE`, list scrolling, complete article scrolling, `16 px` article text, responsive infobox, Italian switching and zero Browser warning/error. Focused `10/10` and complete `541/541` tests pass. Publish and then run the revised `R10-00A`–`R10-05` in the Tesla. |
