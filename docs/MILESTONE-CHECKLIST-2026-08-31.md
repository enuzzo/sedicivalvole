...
| 7 | OFFICE + TESLA | [ ] OFFICE PASS `d45f8dd` · CANONICAL + TESLA RETEST | Finish Soundtrack mechanics and transport access | The existing production deck, source switching, complete Illobo/Jamendo catalogues, three-deck transitions, Music layouts and transport recovery remain the baseline. A compact persistent bottom transport now shares previous / play-pause / next with supported Media Session actions; unsupported actions fail harmlessly. Every committed track change shows a large bounded notice with title, album/source, and artist without firing for stale or failed targets. | Existing `R7-01`–`R7-09` plus `R7-10`–`R7-12`; deterministic handlers and exact local `773 × 601` touch/occlusion QA pass. Real Tesla proof for browser-exposed previous/next controls remains open. |
| 8 | OWNER GATE → OFFICE + TESLA | [ ] OFFICE PASS `d45f8dd` · CANONICAL + TESLA PENDING | Final Illobo identity, playback title, and efficient covers | Both approved supplied marks retain their four-second continuous dissolve and active fixed playback exposes `16 - Artist - Track title`. All 29 drawer covers now use matched public WebP derivatives; their 512 px PNG masters remain local outside the published tree. | WebP headers/mapping/weight assertions pass; served covers total about `380 KB` versus about `9.6 MB` of local masters. Canonical identity and `R8-01`–`R8-03` remain. |
| 9 | OFFICE → TESLA | [ ] STARTUP OFFICE PASS `d45f8dd` · CANONICAL + TESLA RETEST | Accept ATLAS Drive Lab | The owner-approved vertical Drive Lab remains unchanged. ATLAS now prepares from the best truthful point, exposes an explicit waiting/degraded state instead of an unexplained black field, refines without restart, and receives App-scoped session journey history across visual remounts while remaining separate from DISCOVER. | Deterministic cold-start/remount/privacy checks, complete `485/485`, production build and exact local `773 × 601` QA pass. Run `R9-01`–`R9-07` in the Tesla. |
...
| 10A | OFFICE → TESLA | [ ] OFFICE PASS `d45f8dd` · CANONICAL + TESLA PENDING | Close 2026-09-01 drive-note corrections | Truthful ATLAS start/refinement, larger Tesla palette, persistent transport, supported Media Session handlers, committed-track notice, cancelable down/right drawer dismissal and measured Illobo WebP delivery are implemented. | Complete `485/485`, production build and exact local `773 × 601` flows pass. Canonical publication plus `R1-02`, `R7-10`–`R7-12`, `R8-03`, `R9-06`, and `R13-00` remain. |
| 10B | OWNER DETAIL → OFFICE → TESLA | [ ] OFFICE PASS `d45f8dd` · CANONICAL + TESLA PENDING | Close 2026-09-01 17:52–18:07 drive notes | Discover now keeps an in-place navigation QR; Jamendo is header-light and larger; the running Visual drawer is two-column/no-scroll; APERTURE smooths raw low-speed wall input and removes per-frame layout reads; safe coordinate-free preferences and reset actions are implemented; GPS journey continuity is App-scoped and session-only. | Exact local `773 × 601`, safe-storage/privacy/remount tests, complete `485/485` and production build pass. Canonical publication plus `R1-03`, `R5-06`–`R5-07`, `R7-13`, `R9-07`, and `R10-06` remain. |
| 11 | OWNER GATE → OFFICE → TESLA | [ ] NEXT AFTER 10A/10B CANONICAL · THREE-DIRECTION GATE | Original Gradient Field | The approved eighth Original Gradient Field begins only after the drive-note checkpoint reaches canonical PASS: present exactly three original Swiss-influenced directions, admit the owner's choice through fresh licence/performance review, implement it with project-authored rendering, and validate OPEN/UNDERWATER/BLOOM plus full speed range. The completed session contract remains explicit: one app-level collector must continue for the whole running session without coordinate persistence. | Mandatory correction register canonical; selected direction, source boundary, performance tests, exact viewport and Tesla PASS. |
