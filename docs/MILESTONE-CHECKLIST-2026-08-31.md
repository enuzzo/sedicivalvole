...
| 9 | OFFICE → TESLA | [ ] HIERARCHY LIVE `20260901-2012` · STARTUP CORRECTION + TESLA RETEST | Accept ATLAS Drive Lab | Checkpoint `7c9df06` restores the owner-approved vertical instrument: trip summary; full-width Accel/Braking; low Speed-band strip; full-width Heading; full-width Elevation; low Moving/Stopped strip. Histories now have smoothed curves, direct legends and labelled axes. Preserve its shared ranges, icon-only panel handle, route and DISCOVER separation. Still fix the owner-observed black ATLAS cold start: begin map preparation from the best session position available even when accuracy is not yet ideal, render a truthful waiting/degraded state instead of an unexplained black field when no usable position exists, and refine/recenter from later fixes—including the first metres of motion—without restarting or losing the route. | `544/544`, protected build/publication, canonical byte identity and exact live `773 × 601` range/collapse/reopen QA pass for build `20260901-2012`. Run `R9-01`–`R9-06`: `R9-03`–`R9-05` cover real moving traces and cabin legibility; `R9-06` covers immediate map/waiting-state behavior before movement. |
...
| 10B | OWNER DETAIL → OFFICE → TESLA | [ ] MANDATORY · QUEUED AFTER 10A | Close 2026-09-01 17:52–18:07 drive notes | Replace Discover's outbound Maps navigation with an in-place destination QR and concise official Tesla-app sharing guidance; refine Jamendo into a larger, header-light, vertically separated Pace/Genre and clearer Now Playing composition; make the running Visual drawer a two-column description-led no-scroll surface for the current seven choices and the eighth Original Gradient Field admitted by row 11; diagnose and remove APERTURE's low-speed wall-motion stutter; restore only safe product preferences across history/reload and expose CLEAR SAVED STATE in splash and settings. GPS “position” is now clarified separately: after permission, one app-level collector must continue for the whole running session regardless of selected visual, retaining the latest reliable point plus bounded route/journey history only in memory so ATLAS and DISCOVER open already hydrated after visual switches. No coordinate enters persistent storage, diagnostics, or automatic telemetry. | Planned Tesla codes `R1-03`, `R5-06`–`R5-07`, `R7-13`, `R9-07`, and `R10-06`; exact `773 × 601` Browser QA, performance evidence, safe-storage schema/reset tests, session-continuity/remount tests, canonical publication, then target-Tesla acceptance. |
