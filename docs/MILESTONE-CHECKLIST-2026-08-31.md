...
| 5 | TESLA | [ ] IN PROGRESS — ATLAS PERFORMANCE + MERIDIAN MOTION RETEST | Complete visual/performance acceptance | The received report proves ATLAS/JUNCTION at `23.15 FPS` average, `43.75 ms` median and `51.7 ms` p95 against the deliberate 30 FPS ceiling, while Drivey and Vertigo remain near 60 FPS. The ATLAS mitigation caps only the MapLibre framebuffer at `1.25×`, disables world copies, cancels obsolete zoom-tile requests, and consolidates marker updates. After the owner reported stepped MERIDIAN braking and insufficient FOV deformation, checkpoint `6a90621` added shared frame-rate-independent `0.30 s` acceleration / `0.62 s` braking and `0.32 s` visual-FX easing, widened base FOV `50° → 116°`, and requires at least `10°` of change in every `26 km/h` band. Build `20260831-2034` passes `519/519`, 147-module App / 71-module LAB / Sites build, exact local `773 × 601` endpoint QA, protected publication, canonical byte identity and no live warning/error. | Run `R5-01`–`R5-05`, especially `R5-02` through a real progressive `0 → 130 → brake` drive. Capture exact `773 × 601` visual evidence, frame/thermal report, touch/reach notes and no audio degradation; MERIDIAN must show continuous FOV/braking motion, and ATLAS must reach stable measured 30 FPS or the target must be explicitly revised. |
