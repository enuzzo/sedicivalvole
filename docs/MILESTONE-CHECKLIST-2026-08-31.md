...
| Fixed Soundtrack | [x] CANONICAL BASELINE · LOCAL MEDIA HARDENING `[6cda7ee]` · TESLA PENDING | The equal **Play the Road** / **Soundtrack** drawer, true 29-track Illobo Featured catalogue, separate Jamendo Library with 15 live-verified genre routes, authored `1×`, three-deck media, and global MUTE/FX are implemented. Stable native/on-screen Play, Pause, Previous, and Next share one cancellable serialized transport; duplicate buffering Play coalesces, pause/resume keeps position, newer source/track/score/reset intent cancels stale work, and Media Session publication remains truthful. Native invocation IDs/order/outcomes enter the flight recorder. The aggregate `615/615` local gate and production build pass. | Publish and run `R10C-03`, `R4-08`, and `R7-14` in the Tesla; native mini-player behavior and physical listening remain open. |
...
| 13 | OWNER GATE → OFFICE → TESLA | [ ] LOCAL IMPLEMENTATION PASS `[6cda7ee]` · CANONICAL/TESLA PENDING | Shared LIGHT/DARK/AUTO appearance and network state | Direction 2 Road Sheet is implemented with LIGHT default and persisted official-icon LIGHT/DARK/AUTO control. AUTO uses a genuinely observable system scheme first, then an existing consented session position for bounded solar phase, and otherwise a safe no-prompt fallback. Semantic chrome remains independent from the visual palette; the existing enlarged palette and quiet healthy-network state remain intact. | Aggregate `615/615`, exact `773 × 601` LIGHT, `702 × 546` DARK, and `1280 × 720` DARK Browser QA, persistence, focus, safe-storage, AUTO/palette separation, and clean fresh console pass. Publish, then run `R13-00`–`R13-03` in the target Tesla. |
