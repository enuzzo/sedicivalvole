...
| Fixed Soundtrack | [x] OFFICE IMPLEMENTED · CORRECTED LIVE | The equal **Play the Road** / **Soundtrack** drawer, Illobo Featured, Jamendo Library, 30-minute rotation, immediate passenger pace/genre/track selection, authored `1×`, transient three-deck media, and global MUTE/FX are live. Checkpoint `2dd3cb5` adds audible equal-power transition wiring, audio-clock attribution, and the compact QR; `05a754b` adds both final Illobo marks plus the Tesla-facing playback title; `1171157` separates Featured from the initial Library path; `4b36069` closes the target-Tesla exact-ID, activation, failed-deck and UNDERWATER-onset failures; `61471e8` gives every Featured press a random non-repeating start without dropping playlist entries. | Retest failed `R7-06`–`R7-07`; run the remaining `R7-01`–`R7-05` and `R8-01`–`R8-02` in the Tesla. |
...
| 4 | TESLA | [ ] FIELD FAIL CORRECTED · RETEST | Complete adaptive and Soundtrack listening | The owner confirmed UNDERWATER on adaptive playback in build `20260831-0853`, then reported that Jamendo build `20260831-1653` showed the brake effect badge but sounded completely dry. The cause was a linear `18 kHz → 520 Hz` sweep whose `0.4` engagement remained near `11 kHz`. Checkpoint `4b36069`, live in build `20260831-1714`, uses a perceptual logarithmic sweep below `5 kHz` at engagement. Exercise FRACTURE, JUNCTION, NIGHTSHIFT, OPEN, BLOOM, UNDERWATER, global FX, all four manual Soundtrack effects, and vehicle alerts. | `R4-01`–`R4-06` and `R7-06`; per-mode PASS/FAIL matrix; no click, silence, masking, surprise level jump, inaudible visible effect, or stuck processing. |
...
| 7 | OFFICE + TESLA | [ ] FIELD FAILS CORRECTED · CANONICAL PASS; TESLA RETEST | Finish Soundtrack mechanics | The production deck applies the nominal `450 ms` equal-power model, atomic audio-clock attribution and strict three-element ceiling. Earlier corrections `8f03b34`, `dcb6801`, and `1171157` protect metadata, transport activation and the distinct Featured queue. Build `20260831-1653` then failed every Illobo start in the Tesla because catalogue-admitted track `1187970` failed the production exact-ID lookup and its failed deck could poison later transitions. `4b36069` corrects relay/activation/recovery. `61471e8` rotates the complete admitted queue to a fresh random non-current start on every explicit Featured press while preserving all identities and NEXT/PREVIOUS traversal. Run `R7-01`–`R7-07`. | Complete `496/496` and 146-module App / 70-module LAB / Sites build PASS. Build `20260831-1727` passed protected FTP pre/postflight, canonical HTML/JS/CSS byte identity, exact local `773 × 601` three-start interaction/title/overflow/console QA, and a full post-deploy live audit: 50 raw records, 23 policy-admitted, 27 correctly rejected, all admitted **23/23** returning `206 audio/mpeg` with no retry. No MP3 is stored on FTP. `R7-06` and `R7-07` require cabin retest. |
| 8 | OWNER GATE → OFFICE + TESLA | [ ] OFFICE + CANONICAL PASS; TESLA PENDING | Final Illobo identity and Tesla playback title | Both approved supplied marks replace the provisional cover. They dissolve continuously over four seconds in each direction between a white-on-black solid state and the original black-on-graphite outline state, on an unclipped square field with no cover border or radius. Active fixed playback exposes `16 - Artist - Track title` through the page title and restores the product title on pause. | Base `05a754b`; perceptual correction `6218f98`; retained in current build `20260831-1727`; byte-identical assets and canonical HTML/JS/CSS; notice/provenance update; complete `496/496` suite; 146-module App / 70-module LAB / Sites build; exact local `773 × 601` interaction/title/overflow PASS and prior live endpoint/timing/title/console PASS. Run `R8-01`–`R8-02` in the Tesla. |
| 9 | OWNER GATE → OFFICE → TESLA | [ ] OFFICE + CANONICAL PASS; TESLA PENDING | Finish ATLAS road/cardinal overlay | The owner selected direction 1, Navigator Plaque, and requested a dynamic arrow beside the cardinal state. Checkpoint `79d9c9b` implements one compact surface with a filled continuously rotating arrow, English cardinal, exact degrees and a road name read only from rendered `transportation_name` vector data. It replaces the duplicate MapLibre compass, adds no reverse geocoding, and leaves the accepted route/dot treatment unchanged. | Three-direction selection recorded; Navigator-specific `493/493` checkpoint and current complete `496/496` suite pass with 146-module App / 70-module LAB / Sites build; current build `20260831-1727` retains the byte-identical Navigator implementation; exact live `773 × 601` QA proves `SE 135° → S 184°`, road copy, zero overflow and no live warning/error. Run `R9-01`–`R9-02` in the Tesla. |
