...
| 7 | OFFICE + TESLA | [ ] CORRECTION `8f03b34` VERIFIED LOCALLY; REPUBLISH + TESLA PENDING | Finish Soundtrack mechanics | The production deck applies the tested nominal `450 ms` equal-power model through normal changes, reversals and third-deck retargets while preserving the strict three-element ceiling. Audio-clock attribution shows every genuinely audible credit, and the compact QR opens the exact current public track page without exposing the stream URL. Live build `20260831-1219` exposed a failed-incoming-play race and is rejected; `8f03b34` makes target metadata/credit commit atomic. Republish, repeat rapid live QA, then run `R7-01`–`R7-06`. | Focused transition/deck/attribution tests plus the new failure rollback pass; complete `485/485` and 145-module App / 70-module LAB / Sites build PASS. Corrective canonical/live Browser QA and target-cabin PASS remain open. |
