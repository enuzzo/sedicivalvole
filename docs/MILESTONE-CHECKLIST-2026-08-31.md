...
| Fixed Soundtrack | [x] REGRESSION FIX LIVE BUILD `20260902-1954` · TESLA PENDING | The equal **Play the Road** / **Soundtrack** drawer, true 29-track Illobo Featured catalogue, separate Jamendo Library with 15 live-verified genre routes, 30-minute rotation, immediate passenger selection, authored `1×`, three-deck media, and global MUTE/FX are implemented. Checkpoint `1ef48be` makes the audible graph and adaptive macro detector share one AudioContext and bounds every track start with rollback/retry; `0f4a501` retains the readable eight-effect `2 × 4` surface. Exact local/canonical `773 × 601` transport and diagnostic QA plus deterministic pending-promise tests pass. | Run `R4-08` and `R7-14` in the Tesla, then complete `R4-07` and the remaining Soundtrack tests. |
...
| 7 | OFFICE + TESLA | [ ] REGRESSION FIX CANONICAL `20260902-1954` · TESLA RETEST | Finish Soundtrack mechanics and transport access | The production deck, source switching, complete Illobo/Jamendo catalogues, three-deck transitions, Music layouts, persistent transport, Media Session controls and track notices remain the baseline. Checkpoint `1ef48be` removes the dual-AudioContext macro split and gives initial play, selection, NEXT, PREVIOUS and natural end a ten-second wall-clock start deadline with atomic rollback, incomplete-target disposal, late-stale silence and retry. | Complete `553/553`, deterministic never-settling/late-resolution coverage and exact local/live `773 × 601` shared-context plus `NEXT → PREVIOUS → NEXT` QA pass. Run `R4-08` and `R7-14`; real Tesla listening/network behavior remains open. |
...
| 10C | OFFICE → TESLA | [ ] CANONICAL PASS `20260902-2127` · TESLA/PHONE PENDING | Close 2026-09-01 evening–09-02 vehicle notes | Safe preferences/reset remain coordinate-free; Soundtrack no longer blocks START while remote data is pending and automatically joins after preparation; adjacent audio and covers are prefetched; native previous/next remain exposed; transport retracts with the footer; Soundtrack telemetry reduces to speed; Discover uses a `38 px` heading, `246 px` rail and narrower numeral track; its QR opens exploratory Maps search; APERTURE corner cuts share one origin; and Drivey, PRTCL and Gradient state their internal choice counts. The navbar shows only an honest browser connection estimate, not invented cellular bars or continuous pings. Follow-up `92e581b` ensures vehicle motion and passive pointer movement never wake resting chrome; only deliberate pointer press/touch or intentional keyboard focus does. | Focused retraction `32/32`, complete `580/580`, 235-module App / 159-module LAB build, protected publication, byte identity and rendered local/canonical `resting → ArrowUp → resting → visual click → awake` QA pass. Run `R10C-01`–`R10C-08` on target Tesla/phone; physical `R10C-04` remains open. |
...
| 13 | OWNER GATE → OFFICE | [ ] CONFIRMED · NOT IMPLEMENTED | Shared LIGHT/DARK/AUTO appearance and network state | Apply the existing Road Sheet anatomy to DARK and expose the already-prepared independent `LIGHT / DARK / AUTO` model through a direct trigger in the measured free top-rail lane. Manual choices persist; AUTO follows supported browser appearance first and the prepared bounded solar fallback second, without recolouring the active visual palette or storing coordinates. Enlarge the existing footer palette for Tesla before/with this work, while reserving space for the independent appearance trigger, and select the visible quiet-when-healthy network treatment. Retain the black Tabler Now Playing source icon: DARK currently presents it in white non-destructively, while LIGHT must use the black source with the same one-line label contract. | Dark-drawer contrast remains live in final build `20260902-2322` from `543978e` without implementing the shared appearance model. `FI-005` / X10 remains PROMOTED; three-state preference/reset/storage/offline/twilight QA, distinct appearance-vs-palette tests, exact Tesla/desktop layouts, planned `R13-00`–`R13-03`. |
