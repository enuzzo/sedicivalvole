...
| 4 | OFFICE → OWNER UI GATE → TESLA | [ ] SECOND FIELD FAIL REPAIRED LOCALLY · UI/DEPLOY/RETEST | Complete adaptive and Soundtrack listening | The new target-Tesla report proved UNDERWATER dry on Illobo, Jamendo, and NIGHTSHIFT and all manual effects ineffective on both music sources. Checkpoint `8c53e8d` fixes the actual Soundtrack routing defect—macros were read from nonexistent top-level snapshot fields and normalized to zero—then gives Soundtrack and NIGHTSHIFT one two-stage perceptual brake near `1.5 kHz` at visible engagement. Manual flanger, reverb, chorus, and replacement Echo now share one limited post-source graph across Play the Road and Soundtrack. The full `514/514` local suite and App/LAB/Sites build pass; decoded real Illobo and Jamendo excerpts lose `20.8 dB` and `18.8 dB` above `4 kHz` at threshold. Select and implement one of the three manual-FX overlay directions, publish, then exercise every music/effect combination in the car. | Owner-selected overlay and exact viewport QA; canonical build identity; `R4-01`–`R4-06` and `R7-06`; per-mode PASS/FAIL matrix with no click, silence, masking, surprise level jump, inaudible visible effect, clipping, or stuck processing. |
