...
| Fixed Soundtrack | [x] LIVE BASELINE · EIGHT-EFFECT OFFICE PASS · PUBLICATION PENDING | The equal **Play the Road** / **Soundtrack** drawer, true 29-track Illobo Featured catalogue, separate Jamendo Library with 15 live-verified genre routes, 30-minute rotation, immediate passenger selection, authored `1×`, three-deck media, and global MUTE/FX are implemented. Checkpoint `c7ef484` replaces the old four-effect deck with eight shared post-source processors in a readable `2 × 4` surface; exact local `773 × 601` QA and an objective browser audio render pass. | Publish checkpoint `c7ef484`, run `R4-07` in the Tesla, then complete mandatory `10A` and the remaining Soundtrack tests. |
...
| 4 | OFFICE → TESLA | [ ] OFFICE PASS `c7ef484` · CANONICAL PUBLICATION PENDING · TESLA RETEST | Complete adaptive, Soundtrack, and Performance FX listening | The shared post-source graph now contains exactly Flanger, Reverb, Echo, Underwater, Phaser, Bitcrush, Bass Drive, and Radio Cut; Chorus is absent. Manual and vehicle Underwater parameters are separately namespaced, every wet branch initializes neutral, independent depth/RESET/cross-source persistence/authored `1×` remain intact, and a final compressor bounds the complete serial sum. The footer trigger is now a compact `FX ↑` control and the deck is a no-overflow `2 × 4` surface at `773 × 601`. | Deterministic DSP/routing/teardown checks pass; the complete suite passes `543/543`; the production build passes; a real browser `OfflineAudioContext` proves all eight individual differences and the hostile full sum finite/non-silent with `0.55443` peak; exact `773 × 601` interaction/reset/focus QA passes. Publish canonically, then run revised `R4-07` across all five music sources in the Tesla. The old four-effect `R7-06` remains historical baseline evidence. |
...
| 10A | OFFICE → TESLA | [ ] MANDATORY · QUEUED AFTER ROW-4 PUBLICATION | Close 2026-09-01 drive-note corrections | Row 4's eight-effect FX Deck is implemented at office checkpoint `c7ef484`; after its canonical publication, close the cross-cutting notes recorded at 09:01–09:15: ATLAS truthful cold start/refinement; larger Tesla footer palette; persistent previous/play-pause/next transport; Media Session previous/next integration where the Tesla browser exposes it; direction-aware swipe dismissal for every closable drawer/surface with iPhone-like threshold and cancel behavior; large track-change notice; measured WebP Illobo artwork delivery. These are accepted product requirements, not optional future ideas. | Office implementation/tests and canonical publication for every item; planned Tesla codes `R1-02`, `R4-07`, `R7-10`–`R7-12`, `R8-03`, `R9-06`, and `R13-00`; no item may disappear merely because row 11 begins. |
