...
| Fixed Soundtrack | [x] OFFICE IMPLEMENTED · LIVE BASELINE `20260901-0933` · NEW UI PUBLICATION PENDING | The equal **Play the Road** / **Soundtrack** drawer, true 29-track Illobo Featured catalogue, separate Jamendo Library with 15 live-verified genre routes, 30-minute rotation, immediate passenger selection, authored `1×`, three-deck media, and global MUTE/FX are implemented. Owner checkpoint `8450109` supersedes the temporary Music Navigator Rail with two horizontal top selectors and records the selected **Generated image 35** Play the Road composition: sampled JUNCTION/NIGHTSHIFT side by side and responsive-generative FRACTURE full-width below. Exact local `773 × 601` QA proves both panes fit without scrolling. | Publish checkpoint `8450109`, then run `R7-01`–`R7-09`, especially degraded-network `R7-03`, effects `R7-06`, source round-trip `R7-07`, compact-surface acceptance `R7-08`, and source-selector/player acceptance `R7-09`; also run `R8-01`–`R8-02`. |
...
| 10 | OWNER GATE → OFFICE → TESLA | [ ] OFFICE + CANONICAL PASS · TESLA PENDING | DISCOVER passenger companion | Direction 1 is selected and implemented as a split Passenger Index: five image-led places plus `+N MORE` at left, one always-open reader at right, automatic browser language, 14-language selector, search, Nearby/Ahead/Region, bounded continuation to 15 sources, approximate distance/time, Google Maps directions and reciprocal ATLAS action. The same `0.05°` session-only privacy boundary remains; no location enters storage or diagnostics. | Build `20260901-0933` passes protected publication, canonical byte identity and exact live `773 × 601` Browser QA for 15-result expansion, English/Italian switching, Berchet search, reader and Maps destination. Run `R10-01`–`R10-05` in the Tesla. |
