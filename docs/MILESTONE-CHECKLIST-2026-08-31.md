...
| 7 | OFFICE + TESLA | [ ] CORRECTION `dcb6801` VERIFIED LOCALLY; REPUBLISH + TESLA PENDING | Finish Soundtrack mechanics | The production deck applies the tested nominal `450 ms` equal-power model through normal changes, reversals and third-deck retargets while preserving the strict three-element ceiling. Audio-clock attribution shows every genuinely audible credit, and the compact QR opens the exact current public track page without exposing the stream URL. Build `20260831-1219` exposed non-atomic metadata and build `20260831-1229` exposed delayed incoming playback after effects readiness; both are rejected. `8f03b34` makes metadata atomic and `dcb6801` starts both audible decks inside the transport gesture. Republish, repeat normal and rapid live QA, then run `R7-01`–`R7-06`. | Failure rollback plus transient-user-activation ordering tests pass; complete `486/486` and 145-module App / 70-module LAB / Sites build PASS. Corrective canonical/live Browser QA and target-cabin PASS remain open. |
