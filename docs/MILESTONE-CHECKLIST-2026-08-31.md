...
| 11 | OWNER GATE → OFFICE → TESLA | [ ] NEXT · THREE-DIRECTION GATE | Original Gradient Field | The approved eighth Original Gradient Field now begins after canonical PASS of the drive-note checkpoint: present exactly three original Swiss-influenced directions, admit the owner's choice through fresh licence/performance review, implement it with project-authored rendering, and validate OPEN/UNDERWATER/BLOOM plus full speed range. The completed session contract remains explicit: one app-level collector must continue for the whole running session without coordinate persistence. | Mandatory correction register is canonical; selected direction, source boundary, performance tests, exact viewport and Tesla PASS remain. |
