...
| 9A | OFFICE → TESLA | [ ] IMPLEMENTED `febbba3` · CANONICAL/TESLA PENDING | Accept ATLAS semantic map colours | Use the map-local `MAP COLOR` button to switch `PALETTE ↔ STANDARD` with Drive Lab open and collapsed. STANDARD must distinguish water, vegetation/parks, buildings, local roads and major roads while route, marker and UI remain product-palette owned. Reload and RESET SAVED STATE. | Exact local `773 × 601` PALETTE/STANDARD renders, paint-model/persistence checks, complete suite and production build pass. Publish, verify canonical byte identity, then record physical Tesla colour, glare, legibility and touch PASS/FAIL under `R9A-01`. |
