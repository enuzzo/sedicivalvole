...
| 9 | OWNER GATE → OFFICE → TESLA | [ ] OFFICE + CANONICAL PASS · TESLA PENDING | Finish ATLAS Navigator and passenger panel | The owner selected Navigator Plaque direction 1, then selected Live Navigator direction 3 for the passenger revamp. Checkpoints `79d9c9b` and `3ce5c98` provide the dynamic arrow/cardinal/degrees/rendered road plaque plus a map-dominant `272 px` panel split into LIVE MOTION and WHERE YOU ARE. It exposes honest GPS speed/altitude, bounded session distance/time, two-minute speed/altitude trends, locality, road, compact Wikipedia context, two nearby choices and QR. Missing altitude stays `— m`; the persistent Tabler-chevron `HIDE INFO / SHOW INFO` handle is `42 × 116 px`, not a full-height rail. | Build `20260901-1414` at `e24e753` passes focused `32/32`, complete `537/537`, 148-module App / 71-module LAB / Sites build, protected publication, 29/29 Illobo hashes, canonical five-asset byte identity and exact live `773 × 601` QA. Panel fit is `465/465 px`; open/collapsed/reopened geometry, localized place content, nearby rows, QR and full-map restoration pass. Run `R9-01`–`R9-04` in the Tesla. |
