...
| 10 | OWNER GATE → OFFICE → TESLA | [ ] OFFICE PASS `5b4e776` · CANONICAL/TESLA PENDING | DISCOVER passenger companion | Direction 1 remains the independent split Passenger Index. The measured rail fills the available fold before an exact inline `+N MORE`, keeps all 15 results in one scroll, and retains automatic plus explicit language selection. Checkpoint `cdccbd7` embeds the complete localized native Wikipedia Minerva page. Checkpoint `5b4e776` makes free-text search global in the selected edition, relevance-ranked and independent from GPS, while an empty query preserves session-only `NEARBY / AHEAD / REGION`. DISCOVER remains independent from ATLAS; Google Maps remains destination-only. | Focused Discover `12/12`, complete `574/574`, the 235-module App / 159-module protected-LAB / Sites build and rendered no-GPS `Tokyo Tower → result 01 → complete article` QA pass with an empty warning/error log. Publish the checkpoint, then run revised `R10-00A`–`R10-06` in the Tesla, especially global-search test `R10-02` and native-reader test `R10-03`. |
