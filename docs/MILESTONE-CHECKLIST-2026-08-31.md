...
- [x] present compact, equal-weight **Illobo Featured** and **Jamendo Library**
  alternatives using both supplied Illobo identity variants as a three-second
  crossfading Featured cover;
...
| 7 | OFFICE + TESLA | [ ] OFFICE + CANONICAL PASS; TESLA PENDING | Finish Soundtrack mechanics | The production deck applies the tested nominal `450 ms` equal-power model through normal changes, reversals and third-deck retargets while preserving the strict three-element ceiling. Audio-clock attribution shows every genuinely audible credit, and the compact QR opens the exact current public track page without exposing the stream URL. Rejected builds `20260831-1219` and `20260831-1229` exposed non-atomic metadata and delayed incoming playback. `8f03b34` makes metadata atomic; `dcb6801` starts both audible decks inside the transport gesture; build `20260831-1241` publishes both. Run `R7-01`–`R7-06`. | Failure rollback and transient-user-activation ordering tests pass; complete `486/486` and 145-module App / 70-module LAB / Sites build PASS. Protected 108-file publication, pre/postflight, canonical byte identity and catalogue/audio relay HTTP probes PASS. Browser layout/build/log QA passes, but automated live transport is not claimed because Browser control blocked `.php` access and detached. Target-cabin PASS remains open. |
| 8 | OWNER GATE → OFFICE + TESLA | [ ] OFFICE PASS; CANONICAL + TESLA PENDING | Final Illobo identity and Tesla playback title | Both approved supplied marks replace the provisional cover and crossfade every three seconds. Active fixed playback exposes `16 - Artist - Track title` through the page title and restores the product title on pause. | `05a754b`; byte-identical asset hashes; notice/provenance update; complete `490/490` suite; 146-module App / 70-module LAB / Sites build; exact local `773 × 601` timed-cover, title, overflow and console PASS. Publish canonically, then run `R8-01`–`R8-02` in the Tesla. |
