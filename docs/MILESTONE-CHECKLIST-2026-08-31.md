...
| Fixed Soundtrack | [x] LIVE BASELINE · MANDATORY EXTENSION OPEN | The equal **Play the Road** / **Soundtrack** drawer, true 29-track Illobo Featured catalogue, separate Jamendo Library with 15 live-verified genre routes, 30-minute rotation, immediate passenger selection, authored `1×`, three-deck media, and global MUTE/FX are implemented. Owner checkpoint `8450109` records the selected **Generated image 35** Play the Road composition; exact canonical `773 × 601` QA proves both panes fit without scrolling. | The four-effect/manual-transport baseline is not final. Complete mandatory row 4 and `10A`, then run existing `R7-01`–`R7-09` plus planned `R4-07`, `R7-10`–`R7-12`, and `R8-03`. |
...
- [ ] replace the four-effect baseline with the mandatory eight-effect
...
- [ ] expand FX Deck to a readable `2 × 4` touch layout, preserve independent
...
runs first; row `10A` then closes the new drive-noted product defects before
row 11 can begin.
...
| 4 | OFFICE → TESLA | [ ] MANDATORY OFFICE CORRECTION · CANONICAL + TESLA RETEST | Complete adaptive, Soundtrack, and Performance FX listening | Checkpoint `8c53e8d` repairs the common audio path and checkpoint `0993e92` proves the current four-pad FX Deck baseline. That baseline is no longer the completion target. The owner requires eight effects in a `2 × 4` deck: retain Flanger, Reverb, and Echo; remove Chorus; add five distinct processors including progressive manual Underwater plus deliberate low/high-frequency transformations; and make `100%` highly distorted and immediately recognizable without clipping, runaway feedback, silence, or destructive level jumps. Independent depth, RESET, Play the Road/Soundtrack persistence, authored `1×`, global limiting, and the non-modal footer surface remain mandatory. No exact five-effect roster is waiting on the owner: the agent is authorized to design and justify it. | New deterministic DSP/routing/teardown tests, objective rendered-audio difference and level bounds, exact `773 × 601` `2 × 4` layout/interaction QA, protected canonical publication, then revised `R4-07` across all five music sources in the Tesla. The old four-effect `R7-06` remains historical baseline evidence, not final acceptance. |
...
| 10A | OFFICE → TESLA | [ ] MANDATORY · NOT STARTED | Close 2026-09-01 drive-note corrections | Complete row 4's eight-effect FX Deck first, then close the cross-cutting notes recorded at 09:01–09:15: ATLAS truthful cold start/refinement; larger Tesla footer palette; persistent previous/play-pause/next transport; Media Session previous/next integration where the Tesla browser exposes it; direction-aware swipe dismissal for every closable drawer/surface with iPhone-like threshold and cancel behavior; large track-change notice; measured WebP Illobo artwork delivery. These are accepted product requirements, not optional future ideas. | Office implementation/tests and canonical publication for every item; planned Tesla codes `R1-02`, `R4-07`, `R7-10`–`R7-12`, `R8-03`, `R9-06`, and `R13-00`; no item may disappear merely because row 11 begins. |
| 11 | OWNER GATE → OFFICE → TESLA | [ ] BLOCKED BY MANDATORY ROW 4 + 10A | Original Gradient Field | After the mandatory Performance FX and 2026-09-01 drive-note corrections reach office/canonical PASS, present exactly three original Swiss-influenced directions, admit one through fresh licence/performance review, implement it with project-authored rendering, and validate OPEN/UNDERWATER/BLOOM plus full speed range. | Mandatory correction register closed; selected direction, source boundary, performance tests, exact viewport and Tesla PASS. |
| 12 | OWNER GATE → OFFICE | [ ] | Catalogue and audiovisual authorship | Decide intentional visual/score pairings once the expanded catalogue is stable; design the grouped direct-selection Visual Library without weakening immediate switching or current fallbacks. The shared drawer system retains the direction-aware gesture contract introduced by mandatory row `10A`. | Owner-approved mapping and navigation; deterministic selection/gesture tests; no overflow. |
