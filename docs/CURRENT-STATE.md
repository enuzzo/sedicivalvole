...
### Nose-mounted terrain view — selection pending

Three directions presented: A Nose Camera (recommended, full terrain with small data strip), B Flight Instruments (same camera with lateral scales), C Dual View (camera plus radar inset). Each uses oblique forward/downward camera, terrain exaggeration 1.25 and the same delayed measured track. No view built before owner selection.

Feasibility research: MapLibre 5 supports raster-dem terrain and calculateCameraOptionsFromCameraLngLatAltRotation; Mapterhorn provides global Terrarium DEM (https://mapterhorn.com/data-access/ and https://mapterhorn.com/attribution/). It would be a reconstructed map camera, not onboard video. Terrain and aircraft height need a common reference: WGS84 geometric altitude differs from terrain MSL; pressure altitude is not precise terrain clearance. Implement a declared approximation or geoid correction, never silent exact clearance. Preserve stale/unknown-altitude and ground states, avoid camera below exaggerated terrain, and disclose adjustments. Satellite imagery is a separate licensed service decision; terrain geometry alone does not imply satellite texture. No terrain provider requests or runtime shipped in this checkpoint.

Physical Tesla/iPhone acceptance and sustained GPU performance remain separate from browser QA.

Validation: 846 native tests passed, followed by 30 focused radar/documentation checks. Chrome at 773 x 601, 874 x 402 and 956 x 440 verifies selected-only single ring, upright distance, exact C172 without track, AS50 category and expanded full type name. Controlled moving traffic advances in all 20 consecutive sampled frames after enabling subpixel marker positioning. No page/console errors; synthetic diagnostic mail blocked. Current runtime WebKit executable is unavailable (installed revision differs); no WebKit pass is claimed. Source/build/publication evidence follows separately.
