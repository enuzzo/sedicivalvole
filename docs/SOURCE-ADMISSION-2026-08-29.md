...
Status: **approved; the DRIVEY boundary was superseded by its source-faithful
integration, and PRTCL now has a directly authorized formula-adaptation
...
InfiniteTubes and Primordial remain original AGPL-licensed project code under
the clean-room boundaries recorded here. PRTCL is different: after inspecting
the rendered source, the repository owner directly authorized reuse and
modification of the three selected formulas. Sedici Valvole adapts those
formulas into its own bounded WebGL2 renderer without importing the PRTCL
runtime, interface, dependencies, assets, fonts, screenshots, or brand. After
the initial Drivey implementation was rejected, the product owner selected a
source-faithful integration; DRIVEY therefore vendors the 51 files required by
the modern upstream runtime, unchanged and separately licensed. Attribution
records provenance and does not infer or relicense a broader upstream grant.
...
| PRTCL | local commit `2a22f33b975e2c40b7ee0bdd2d1acb4cee4f5060` | The audited README states MIT and carried an obsolete studio attribution, but the checkout contains no `LICENSE` file and `package.json` has no licence field. `enuzzo`, as project and repository owner, directly authorized reuse and modification for this work. This record does not infer a broader grant. | GPU adaptations of the Fractal Frequency, Murmuration, and Axiom procedural formulas, their reviewed particle counts, and their characteristic camera compositions. | The PRTCL runtime, original TypeScript preset files and source strings as files, UI, fonts, brand assets, screenshots, dependencies, and all other effects. |
...
| [GLSL: Primordial Soup](https://codepen.io/shubniggurath/pen/NXGbBo) | Pen `NXGbBo`, rechecked `2026-08-29` | The public Pen credits Liam Egan, and [CodePen's public-Pen policy](https://blog.codepen.io/documentation/licensing/) applies MIT by default. The Pen source also contains an attributed value-noise fragment by Inigo Quilez whose separate reuse terms were not established by this audit. | A clean-room, project-authored fluid scalar field based only on the rendered high-level mechanics: layered flow, colour islands, touch deformation, and speed-driven convergence. | The Pen's HTML, CSS, JavaScript, shader text, Three.js runtime, and the attributed noise function. |

## Selected PRTCL effects

- **Fractal Frequency** is the primary PRTCL variant. One compact direct
  `TYPE` control cycles the three admitted families and remains distinct from
  the shared Sedici Valvole palette control. There is no dropdown, panel, or
  exposed low-level particle tuner. Road speed drives particle size, depth, and
  travel; musical level drives colour phase and pulse.
...
1. DRIVEY runs its separately licensed, byte-identical vendor runtime inside a
   same-origin iframe controlled by a project-authored bridge. PRTCL uses a
   project-authored renderer around the directly authorized adaptations above;
   InfiniteTubes and Primordial retain separate clean-room renderer modules.
