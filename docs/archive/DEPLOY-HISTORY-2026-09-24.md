# Deployment — records to September 24, 2026

> This is the complete `docs/DEPLOY.md` as it stood on September 24, 2026, before the procedure and a release table moved to the top of [DEPLOY](../DEPLOY.md). Each record is kept verbatim with its original heading, so older anchors still resolve here.
...
upload=PASS files=232 bytes=254884684
...
## Compact telemetry and visual controls — 2026-09-03 21:56
...
- protected publication: **PASS** after explicit owner approval. Read-only
  preflight passed; the initial `--preserve-existing` upload safely staged the
  fingerprinted assets without switching the root. The approved normal
  publisher then applied its exact hash-gated static-entry and legacy/cache
  cleanup, and the independent read-only postflight reports `network=PASS`,
  `login=PASS`, `directory=PASS`, `root_and_legacy_targets_verified`, nine root
  entries, and `remote_writes=NONE`;
- canonical response: `/` is HTTP/2 200 with `no-store/no-cache`, `nosniff`,
  Nginx/PHP evidence, and cache `MISS`; it now references JavaScript
  `index-gDT2Y9_b.js` and CSS `index-x-RYexQS.css`;
- byte identity: local/live HTML is `1,206` bytes at SHA-256
  `125d139582d0c85193876943799e031996a3682d96af06fdbeadc31fd0228334`;
  local/live JavaScript is `738,526` bytes at SHA-256
  `b501166709c819083e00bbc1c3f1aa4c23c4b5110f6960842bc74f406c621d92`;
  local/live CSS is `164,648` bytes at SHA-256
  `0971fbbb36ece60415ad7133ed881fe56c904a86de9d7d7687f078a0bab8889e`;
- embedded identity: the canonical JavaScript reports build
  **`20260903-2137`** and clean source/documentation checkpoint **`4ffd707`**.
  Local rendered/browser QA is complete; physical-Tesla touch, viewing-distance,
  glare, network interpretation, and continuous audio listening remain separate
  acceptance gates.
...
- build stamp: **`20260901-1823`**; deployed documentation checkpoint:
  **`d3e145c`**; product checkpoint: **`f48b5b2`**;
- behavior: **PASS in exact live Browser QA; Tesla acceptance pending**. Every
  authored tap remains below the stunt boundary, while the final `82–100%`
  adds processor-specific dry subtraction, wet character, modulation,
  filtering or drive. Manual Underwater adds a maximum-depth pressure texture
  without sharing state with automatic braking Underwater;
- tests/build: **PASS**. The complete suite passes `544/544`; the 148-module
  App, 71-module LAB, protected LAB packaging and Sites build pass. A real
  browser `OfflineAudioContext` render measures every full endpoint
  `1.3925–2.5184×` more transformed than its authored tap, manual Underwater
  `1.8005×`, maximum individual peak `0.99121`, and the hostile eight-effect
  sum finite/non-silent at `0.22828` RMS / `0.60941` peak;
- protected publication: read-only preflight reported `remote_writes=NONE`;
  publication uploaded `181` files / `223,322,900` bytes, fully reverified all
  `29/29` Illobo masters, retained one previous content-addressed asset for
  cache overlap, preserved the dynamic root, and reported
  `remote_writes=ROOT_UPLOAD_ONLY`;
- canonical identity: cache-busted HTML is HTTP 200 with `no-store/no-cache`
  and `nosniff`. HTML, main JavaScript and main CSS match the candidate
  byte-for-byte at SHA-256
  `2c53acfcad378f978dc9822b10eaec843b7882d4c2bba5024f352bb7be6acad0`,
...
- exact live `773 × 601` Browser QA: **PASS**. The deck measures
  `720 × 267 px`; all eight sliders reach `100` and report `8/8 ACTIVE`; RESET
...
- remaining acceptance: run `R4-07` across FRACTURE, JUNCTION, NIGHTSHIFT,
  Illobo, and Jamendo in the target Tesla, including authored taps versus
  `100%`, manual immersion/surfacing, hostile pairs, RESET, source switching
  and automatic braking Underwater coexistence.
...
## Gradient, credits and Atlas camera publication — 2026-09-12 10:16
