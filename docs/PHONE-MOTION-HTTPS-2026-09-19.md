...
The existing PHP endpoint remains backward-compatible with legacy direct signaling. HTTPS admission is one-use, expires after three minutes and exchanges independent receiver/phone capabilities. The original 15-second idle lease and offline/three-failure teardown are superseded by the owner's September 20 [road outage recovery](PHONE-NETWORK-RECOVERY-2026-09-20.md). Admitted HTTPS pairing lasts until the original absolute one-hour ceiling; foreground transient faults retry serially with backoff capped at two seconds and unchanged 1.5-second request timeouts. STOP/hide or terminal authorization/expiry still tears down. Recovery uses the admitted capability, never reused QR admission or a background session.
