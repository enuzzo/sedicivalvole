...
Catalogue requests have one 30-second deadline covering response headers and JSON decoding. Replacement, pause and teardown abort obsolete requests and settle their callers even if the provider never responds. A requested catalogue owns the transition if the outgoing recording ends while it is loading. On failure, evaluate current audibility rather than an earlier snapshot; preserve the requested selection for retry when no committed recording remains audible.

Exactly three transient media roles are retained. Current owns `preload=auto`; adjacent roles start metadata-only and only NEXT promotes after current has `30 seconds` of observed contiguous forward buffer. PREVIOUS rewinds/reuses a healthy retained media element instead of destroying its buffer. Each initial/manual/automatic track transition starts its target silently, waits for a six-second buffer floor, rewinds when not already exactly at the authored beginning, then waits for `seeking=false` and at least `HAVE_FUTURE_DATA` before making it audible. All preparation, including post-seek readiness, shares one ten-second transport deadline. Outgoing **committed** track/metadata stay intact during a manual transition; an initial selection stays silent. Pause/replacement/destroy cancel obsolete preparation promptly, and timeout preserves retry/rollback. Rapid transport commands execute in exact order.
...
Exactly eight manual effects share one level-bounded post-source graph across Play the Road and Soundtrack. Retain Flanger/Reverb/Echo; Chorus is removed. Five differentiated additions include progressive manual Underwater and three band effects. Do not restore Beat Repeat's retired worklet. Every `100%` state is unmistakably extreme without clicks, silence, clipping, runaway feedback or destructive level jumps. After the September 24 owner report that the band effects crackled, **Bass Cut**, **Mid Focus** and **High Cut** (ids `bassDrive`, `radioCut`, `highCut`) are clean serial fourth-order Butterworth filters with no waveshaper or parallel dry blend; see the [measurements](../NIGHT-INSTRUMENT-2026-09-24.md#owner-audio-correction). Footer MUTE and FX apply across both sources; FX OFF disables vehicle-macro **audio** processing while preserving the active visual response. References to OPEN/BLOOM in that older FX rule do not restore those retired macros. Preserve independent depth, reset and source-switch persistence in the [non-modal FX Deck](interface.md#running-media-and-effects).
