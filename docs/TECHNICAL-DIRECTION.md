...
The Flux chrome uses a shared 6 px UI radius for framed controls and diagnostic surfaces. The owner-selected **Swiss Compact** system separates type from interaction geometry through one semantic scale: `14 px` metadata, `15 px` labels, `16 px` body/action copy, `18 px` active names, `24 px` titles, and `34 px` primary values. Metadata remains high contrast, live values use tabular numerals, discrete action targets use at least `48 px`, and primary actions use `56 px`; no universal type floor may flatten this hierarchy. At `773 x 601` the retracting navbar, lower Now Playing band, and footer are each `72 px`, preserving a substantially larger uninterrupted visual field. MUTE and FX keep equal tracks and direct `LABEL / ON–OFF` anatomy while obsolete `GLOBAL` and active-count microcopy stays hidden. Structural rails remain aligned to the underlying grid. Functional labels stay uppercase, while editorial Visual, Music, mode, and effect names use separate Title Case display labels in the launcher, footer, and pickers. Stable identifiers and canonical registry labels remain unchanged. A persistent `532 × 72 px` lower Now Playing overlay occupies its own band immediately above the footer whenever chrome is awake, follows every committed fixed-recording identity including automatic changes, and combines `56 px` artwork with `48 / 56 / 48 px` previous/play-pause/next targets. Stable Media Session handlers live for the session and dispatch into one serialized, cancellable transport queue. At the Tesla breakpoint, the exact top grid is `164 / 156 / 60 / 86 / 84 / 116 / 107 px` for mode, speed, network, appearance, GPS, Discover, and Report; the absolute speed readout uses that same cell and `--line` stroke, so hiding the surrounding chrome does not move or restyle it as a separate overlay.
