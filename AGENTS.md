...
- `@shadergradient/react@2.4.20` and its pinned Three/R3F peers are admitted only as development dependencies for the standalone `shadergradient-lab.html` playground and the authenticated protected LAB. Keep them out of the public App, product catalogue, canonical client bundle, and unprotected deployment paths. Any later product integration or upstream modification requires an explicit owner selection, retained MIT notice, exact source/change inventory, fallback coverage, and target-Tesla GPU evidence.
...
- Prefer curated audiovisual environments and purposeful Visual/Music selectors over low-level `Atmos`, `Harmonics`, and `Pulse` controls in the primary driving surface. At `773 x 601`, keep the retracting footer near 64 px, avoid redundant text rows, and use disclosure carets only for controls that open real menus. The user-adjustable energy-threshold slider is retired: normalize visual and music energy against the fixed `130 km/h` legal-road ceiling, while ensuring the tunnel is already visible by approximately `40 km/h`.
