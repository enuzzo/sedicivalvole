...
export function phoneSetup({ sensor = {}, link = {}, hasPair = false, attempted = false } = {}) {
  ...
  if (!sensor.tared || sensor.roadState !== 'calibrated') return result(3,
    ...
    sensor.roadState === 'unsupported-pose' ? 'Tilt the portrait phone back in its holder. Keep it still, then tap ZERO again.' : 'While parked: tap ZERO on your phone, lift your finger and keep still.', 'zero');
...
export function receiverSetup(s = {}) {
  ...
  if (s.state === 'stale' || s.dataFresh !== true) return result(1, 'Waiting for fresh phone data.', 'Check your phone: keep its page visible. Restart setup if data does not return. GPS remains in control.');
  ...
  if (!s.mountSelected) return result(2, 'Position your phone.', 'Secure the phone upright in its holder. Confirm placement on your phone.');
  if (!s.tared || s.roadState !== 'calibrated' || s.tareState === 'settling' || !s.referenceReceived) return result(3, s.roadState === 'unsupported-pose' ? 'Check the phone holder.' : 'Set ZERO on your phone.', 'While parked, keep the phone still and follow its ZERO instructions.');
