...
export function phoneSetup({ sensor = {}, link = {}, hasPair = false, attempted = false } = {}) {
  ...
  if (!(sensor.placementConfirmed ?? sensor.mountSelected)) return result(2, 'Place your phone.', 'While parked: secure your phone. Choose below whether its sensors should control acceleration, braking and Aperture turns.', 'position');
  ...
  if (sensor.mountSelected && ['unsupported-pose', 'moved', 'needs-zero'].includes(sensor.roadState)) return result(3, 'Check car alignment.', 'Keep the screen facing the cabin, aligned straight ahead, upright or inclined. Set ZERO again, or disable aligned car motion in details.', 'zero');
