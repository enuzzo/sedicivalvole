...
import { MotionQuality, MotionNext, MountChoice, MotionSourceStatus } from './motion-ui.jsx';
...
export function useLocalSensors(onEvent) {
  ...
  return { capability, preferred: (capability.needsPermission && capability.touchInput) || capability.observed, summary, activity, sample, start,
    setMount: selected => { owner.current?.setMount(selected); setSummary(owner.current?.summary() ?? {}); },
    stop: () => owner.current?.stop(), zero: () => owner.current?.requestTare() };
}
export function LocalSensorsPanel({ sensors, gpsState, source, responseLabel, themeKey, onRemote, onClose }) {
  ...
  return <div className="motion-panel-content local-sensors-panel">
    ...
    <MountChoice selected={sensors.summary.mountSelected} onChange={sensors.setMount}/>
    ...
    <p>Calibrated holder acceleration feeds Engine and Music; gyro bends Aperture. Handheld mode feeds TRACE only for road audio. Keep the phone fixed in its holder. GPS remains the speed reference; motion is never integrated into an invented speed. No phone pairing is needed.</p>
