    ...
    <section className="launch-cockpit" aria-label="Choose your drive" inert={picker ? true : undefined}>
      ...
      {engine ? <div className="cockpit-engine-body">
        ...
        <div className="cockpit-presets">
          ...
          <div className="cockpit-palette-group"><small>PALETTE</small><button type="button" className="cockpit-palette" aria-label="Next palette" title={`Palette: ${theme.label}`} onClick={onPalette}><i className="cockpit-palette-thumb" style={{background:theme.swatchSecondary ? `linear-gradient(105deg, ${theme.swatch} 0 52%, ${theme.swatchSecondary} 52% 100%)` : theme.swatch}} aria-hidden="true"/><strong>{theme.label.replace(/\s+\d+$/, '').toLowerCase().replace(/^./, character => character.toUpperCase())}</strong></button></div>
        </div>
      </div>}
