...
const paths={
  ...
  parachute:'M2 22C2 0 40 0 40 22L28 20L21 23L14 20ZM4 24H6L19 37H23L36 24H38L24 40V44H18V40ZM20 24H22V37H20Z',
  airship:'M21 2C7 2 7 32 18 37L13 43H29L24 37C35 32 35 2 21 2Z',
...
export function radarCategory(plane,types={}) {
  ...
  if(plane.typeCode==='SHIP')return 'airship';
  if(desc.startsWith('B')||plane.typeCode==='BALL'||plane.category==='B2')return 'balloon';
  if(desc.startsWith('P')||plane.category==='B3')return 'parachute';
  if(['B1','B4'].includes(plane.category)||desc==='L0-')return 'glider';
  if(((desc.startsWith('L1')||desc.startsWith('S1')||desc.startsWith('A1'))&&!desc.endsWith('J'))||(!desc&&plane.category==='A1'))return 'light';
  if((desc.startsWith('L2')||desc.startsWith('S2'))&&!desc.endsWith('J'))return 'twin';
  if(desc.startsWith('D')||plane.category==='B6')return 'drone';
