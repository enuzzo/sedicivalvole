...
const FRAGMENT_SHADER = `#version 300 es
  ...
  void main() {
    ...
    float roadPhase = (v_surface.y - u_travel) * 18.0 + v_surface.z * 4.0;
    float longitudinal = 0.93 + 0.07 * sin(roadPhase);
    float roadFlow = pow(0.5 + 0.5 * cos(roadPhase * 0.5), 7.0) * mix(0.025, 0.075, u_energy);
    ...
    color += v_color * (surfaceSheen + roadFlow);
...
function movingCenter(ribbon, progress, time, travel, profile) {
...
function ribbonPoint(ribbon, progress, time, travel, profile, aspect) {
  const center = movingCenter(ribbon, progress, time, travel, profile);
  const motion = wakeRibbonMotion(ribbon, progress, time, profile, travel);
  ...
  const before = movingCenter(ribbon, Math.max(0, progress - epsilon), time, travel, profile);
  const after = movingCenter(ribbon, Math.min(1, progress + epsilon), time, travel, profile);
...
function buildGeometry(palette, time, travel, profile, aspect) {
  const data = [];
  for (const ribbon of WAKE_RIBBONS) {
    ...
    for (let segment = 0; segment < SEGMENTS; segment += 1) {
      ...
      const points = [ribbonPoint(ribbon, a, time, travel, profile, aspect), ribbonPoint(ribbon, b, time, travel, profile, aspect)];
...
export function createWakeRenderer(canvas, initialPalette) {
  ...
  return {
    ...
    render({ speedKmh, effect, reducedMotion, deltaSeconds }) {
      ...
      const geometry = buildGeometry(palette, time, travel, profile, width / height);
      gl.bindBuffer(gl.ARRAY_BUFFER, buffer);
