...
function Showcase() {
  ...
  return <main className="app design-system" data-appearance={appearance} style={theme.css}>
    ...
    <form className="ds-settings" onSubmit={e=>e.preventDefault()}>
      ...
      <button type="button" aria-pressed={resting} onClick={()=>setResting(v=>!v)}>{resting?'Wake global chrome':'Retract global chrome'}</button>
    </form>
    <ContextualControlsContext.Provider value={!resting}>
    <section aria-labelledby="controls-title"><h2 id="controls-title">Contextual visual controls</h2><p>Same 112 px cells, minimum 48 px targets, 6 px corners, 1 px stroke and left-aligned label/value. Try every button; Tab exposes focus.</p>
      <div className="ds-specimens">
        <article><h3>Drivey</h3><div className="ds-stage"><DriveyCycleControl settings={drivey} onChange={setDrivey}/></div><p>RENDER toggles Normal / Wire.</p></article>
    ...
    </ContextualControlsContext.Provider>
    ...
    <section><h2>Visual inventory</h2><p>A field without a local setting does not acquire a decorative button. Map navigation, live measurements and drawers keep their own semantic roles.</p><div className="ds-table"><table><thead><tr><th>Public visual</th><th>Contextual control</th></tr></thead><tbody>{FLUX_VISUAL_CHOICES.map(v=><tr key={v.id}><td>{v.displayLabel||v.label}</td><td>{v.id==='drivey'?'RENDER':v.id==='prtcl'?'TYPE':v.kind==='family'?'VARIANT':v.id==='atlas'?'Map navigation / places':v.id==='air-atlas'?'Traffic / map / orientation / flight':v.id==='stats'?'Time ranges / observations':v.id==='discover'?'Search / language / articles':'None — shared Visual library'}</td></tr>)}</tbody></table></div></section>
