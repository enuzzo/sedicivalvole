import { AudioManager, type DynamicAudioNode } from "./AudioManager.ts";
import type { Drivetrain } from "./Drivetrain.ts";
import { ratio } from "./util/ratio.ts";
...
export class Engine {
    ...
    integrate(load_inertia: number = 0, time: number, dt: number) {
        ...
        this.omega = Math.max(0, Math.min(this.omega, this.omega_max));
    ...
    applySounds(samples: Record<string, DynamicAudioNode>, gearRatio = 0, rpmPitchFactor = 0.2) {
        ...
        const applySample = (key: string, gain: number, applyPitch: boolean = true) => {
            ...
            if (applyPitch) {
                samples[key].audio.detune.setTargetAtTime(
                    Math.max(-2400, Math.min(2400, this.getRPMPitch(samples[key].rpm, rpmPitchFactor))),
                    samples[key].audio.context.currentTime, 0.025);
            }

            samples[key].gain.gain.setTargetAtTime(gain * samples[key].volume, samples[key].audio.context.currentTime, 0.025);
        };
