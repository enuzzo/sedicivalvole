...
export class Drivetrain {
    ...
    changeGear(gear: number) {
        ...
        // The host commits the ratio once at its scheduled audio-time boundary.
        this.pendingGear = gear;
    }

    pendingGear: number | null = null;

    commitGear() {
        if (this.pendingGear === null) return;
        this.gear = clamp(this.pendingGear, 0, this.gears.length);
