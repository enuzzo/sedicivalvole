...
try {
  ...
  await page.reload();await page.getByRole('button',{name:'START MUSIC',exact:true}).waitFor();assert.doesNotMatch(await page.locator('.cockpit-value').first().innerText(),/Slow/);check('every visit rolls a genre after a precise pace choice');
  ...
  await page.getByRole('button',{name:'Choose visual',exact:true}).click();assert.equal(await page.locator('.cockpit-picker-options>button').count(),8);
  ...
  const previousVisual=await page.locator('.cockpit-visual .cockpit-value').innerText();await page.getByRole('button',{name:'Change visual',exact:true}).click();assert.notEqual(await page.locator('.cockpit-visual .cockpit-value').innerText(),previousVisual);assert.equal(await page.locator('.cockpit-dialog').count(),0);const nextVisual=await page.locator('.cockpit-visual .cockpit-value').innerText();await page.reload();await page.getByRole('button',{name:'START MUSIC',exact:true}).waitFor();assert.notEqual(await page.locator('.cockpit-visual .cockpit-value').innerText(),nextVisual);check('Change rerolls without a dialog; reload excludes previous visual family');
  ...
  const compactHeight=await page.locator('.cockpit-choices').evaluate(el=>el.getBoundingClientRect().height);await page.setViewportSize({width:1280,height:1200});await geometry(page,'Tall Mac 1280x1200');assert.equal(await page.locator('.cockpit-choices').evaluate(el=>el.getBoundingClientRect().height),compactHeight);assert.ok(compactHeight<=150);assert.ok(await page.locator('.cockpit-preset-thumb').evaluateAll(es=>es.every(e=>e.getBoundingClientRect().width===36&&e.getBoundingClientRect().height===36)));await page.screenshot({path:`${out}/10-tall-mac.png`});check('tall windows do not grow selection rows; preset art is 36 px');
