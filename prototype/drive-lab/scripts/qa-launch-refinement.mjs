...
try {
 ...
 await page.getByRole('button',{name:'Choose visual',exact:true}).click();await page.getByRole('button').filter({hasText:/^Air Atlas/}).click();await page.locator('.cockpit-visual .cockpit-thumb').evaluate(e=>e.decode());assert.equal(await page.locator('.cockpit-visual .cockpit-thumb').getAttribute('src'),'/artwork/visuals/air-atlas.webp');await page.screenshot({path:`${out}/dark-aircraft-preview.png`});check('Air Atlas uses the verified real screenshot in dark mode');
