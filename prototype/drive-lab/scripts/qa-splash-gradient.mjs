...
const launch=async()=>{await page.getByRole('button',{name:'Visuals only',exact:true}).click();await page.getByRole('button',{name:'Change visual',exact:true}).click();await page.locator('.cockpit-picker-options button').filter({hasText:'Gradient'}).click();await page.locator('.cockpit-picker-options button').first().click();await page.getByRole('button',{name:/^START/}).click();await page.locator('.shadergradient-field canvas').waitFor();await page.waitForTimeout(1500);};
