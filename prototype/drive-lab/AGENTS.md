...
- Controls are touch-first, flat, and fully legible; the GL field provides the visual contrast. The owner-selected **Swiss Compact** direction replaces the retired blanket Automotive Glance floor with semantic roles shared by the public product, owner LAB, ShaderGradient workbench, diagnostics, attribution, and responsive states: `14 px` metadata, `15 px` labels, `16 px` body/action copy, `18 px` active names, `24 px` titles, and `34 px` primary values. Keep metadata high contrast, use tabular numerals for changing data, keep important action targets at least `48 px` and primary actions `56 px`, and preserve the full ladder rather than collapsing every role to one size. At the verified `773 x 601` viewport, the retracting top bar, lower Now Playing band and footer are each `72 px`; preserve the visual field through hierarchy and deliberate scrolling. Visual/Music open explicit caret-marked libraries rather than cycling blindly.
