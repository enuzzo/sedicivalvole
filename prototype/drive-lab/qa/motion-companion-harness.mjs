...
let phone,receiver,paused=false,joining=false,receipt=null,phoneState=null;
...
document.querySelector('#run').onclick=()=>{
 ...
 phone=createMotionSession({role:'phone',getPhone:()=>({summary:{sensorState:paused?'stale':'live',accelerometer:true,gyroscope:true,orientation:true,tared:!paused,tareCount:1,cadenceHz:50},values:paused?null:{frame:'tare-relative',generation:1,acceleration:[1,2,3],rotation:[0,0,1],tilt:[0,0,0.5],turnRate:1,ageMs:0}}),onChange:s=>{phoneState={state:s.state,sent:s.sent};render();}});
 receiver=createMotionSession({role:'receiver',onChange:s=>{
   receipt={state:s.state,received:s.received,rttMs:s.rttMs,tared:s.tared,samplePresent:Boolean(s.values),sensorState:s.sensorState};render();
