...
function Receiver() {
  ...
  const ready=()=>{Object.assign(fixture,{sensorState:'live',mountSelected:true,tared:true,tareState:'tared',roadState:'calibrated',wakeLock:true,wakeState:'active'});state='connected';set(live());};
  return <main className="app qa-receiver" data-appearance={theme} style={{...resolveSemanticTheme(getFluxTheme('red'),theme).css,color:'var(--ui-text)',background:'var(--ui-surface-strong)',minHeight:'100dvh'}}>
    <nav aria-label="Synthetic QA states" style={{position:'fixed',top:8,left:8,zIndex:40,display:'grid',gap:8,width:80,fontSize:13}}>
      <span>Synthetic preview</span><button onClick={ready}>QA LIVE</button>
