...
opaque.resize((256, 256), Image.Resampling.LANCZOS).save(icon, sizes=[(16,16),(32,32),(48,48)])
files.append(icon)
report = ROOT / 'prototype/drive-lab/public/report-support/report-mark-pistons.png'
report.write_bytes((OUT / 'icon-256.png').read_bytes())
files.append(report)
inventory = {'source': 'logo/pistons-v1/source.png', 'sourceSha256': hashlib.sha256(SOURCE.read_bytes()).hexdigest(),
    'operation': 'Owner-supplied artwork, unchanged geometry; black compositing and Lanczos size conversion only.',
